...
def compile(cores, test, outputs, ignore):
    ...
    if test == Configs.dynamic:
        cmd = ["strc", "-smp", str(cores), "--perftest", "--outputs", str(outputs), '--preoutputs', str(ignore), "--threadopt", '--noiter', '--selective', 'SelectionDynamicPull.str']    
    print ' '.join(cmd)
...
def plot_normalized(cores):
    ...
    with open('./selection-normalized-pull.gnu', 'w') as f:        
        ...
        f.write('set title \"Selection Operator\"\n')
