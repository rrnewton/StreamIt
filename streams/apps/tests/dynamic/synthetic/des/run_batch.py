...
def plot():
    ...
    with open('./des-batch.gnu', 'w') as f:        
        ...
        f.write(cmd)
    os.system('gnuplot ./des-batch.gnu')
    os.system('ps2pdf ' + output)


def plot_normalized():
    data = 'des-batch-normalized.dat'
    output = 'des-batch-normalized.ps'  
    cmd = "plot "
    cmd += "\"" + data + "\" u 1:4 t \'dynamic\' w linespoints, "
    cmd += "\"" + "\" u 1:4:5 notitle w yerrorbars, "
    cmd += "\"" + data + "\" u 1:2 t \'static\' w linespoints,"
    ...
    with open('./des-batch-normalized.gnu', 'w') as f:        
        ...
        f.write('set output \"' + output + '\"\n')
        f.write('set key left top\n');
        f.write('set title \"Synthetic Dynamism DES with Batching Normalized\"\n')
        f.write('set xlabel \"Cores\"\n');
        f.write('set ylabel \"Throughput normalized to static throughput with 1 core\"\n');
        f.write(cmd)
    os.system('gnuplot ./des-batch-normalized.gnu')
    os.system('ps2pdf ' + output)
