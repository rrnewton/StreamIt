...
public class ReedSolomonEncoder extends Filter {
  ...
  */
  ...
  void zero_fill_from (int buf[], int from, int to)
  {
    int i;
...
//    }
  ...
  void build_codeword (int msg[], int nbytes, int dst[])
  {
    int i;
    ...
    for (i = 0; i < NPAR; i++) {
      dst[i+nbytes] = (int)pBytes[NPAR-1-i];
    }
  }
  ...
  void decode_data(int ddata[], int nbytes)
  {
    int i, j, sum;
    for (j = 0; j < NPAR;  j++) {
      sum	= 0;
      for (i = 0; i < nbytes; i++) {
	sum = ddata[i] ^ gmult(gexp[j+1], sum);
      }
      synBytes[j]  = sum;
    }
  }
...
//    }
  ...
  void encode_data (int msg[], int nbytes, int dst[])
  {
    int i;
