...
in_S = x)$ where max operates component-wise.

\item Given that there are $x$ items on the output tape of joiner
$J$, then $g_{ABJ}(x)$ is a pair of values $(y_1, y_b2)$ where $y_1$
and $y_2$ are the minimum number of items on the input tapes of $A$
and $B$, respectively.  That is, $g_{SAB}(x) = min( (in_A, in_B) |
in_S = x)$ where min operates component-wise.

\end{itemize}

{\bf Round robin.}  We first give derivations for round-robin
splitters and joiners.  For a split, 
