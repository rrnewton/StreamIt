...
$n$, where $n$ could is any integer.  In $n$ invocations of $A$'s {\tt
work} function, $A$ will produce one or more data items $d$.  Now, the
...
latency $n$, where $n$ could is any integer.  Let $n(a)$ denote the
number of items on tape $a$.  For the message to be able to be
delivered on time, we have that:

\begin{enumerate}

\item If $B$ is upstream of $A$:
\begin{eqnarray*}
n(O_B) \le \mi{O_B}{O_A}(n(O_A) + push_A * n)
\end{eqnarray*}
That is, $n(O_A) + push_A * n$ is the number of items on $A$'s output
tape after producing the data of interest.  Then, $y =
\mi{O_B}{O_A}(n(O_A) + push_A * n)$ is the latest item on $B$'s output
...
which is only possible if the item count $n(O_B)$ does not exceed $y$,
as specified by the constraint.

\item If $B$ is downstream of $A$:
\begin{eqnarray*}
n(O_B) \le \ma{O_A}{O_B}(n(O_A) + push_A * (n-1))
\end{eqnarray*}
That is, $n(O_A) + push_A * (n - 1)$ is the number of items on $A$'s
output tape before pushing the data of interest, and $z =
\ma{O_A}{O_B}(n(O_A) + push_A * (n-1))$ is the maximum number of items
...
be delivered before this effected data appears on $B$'s output, so the
number of items $n(O_B)$ on $B$'s output must not exceed $z$.
