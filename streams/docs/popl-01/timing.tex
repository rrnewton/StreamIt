...
\caption{The behavior of the tapes in a filter that pop two elements, peeks the next three from the input
channel and pushes two elements to the output channel. The tape of the
input channel before an invocation is denoted in the left. The state of
both input channel tape and output channel tape and the filter during
execution is shown in the middle. The state of the output channel tape
after completion of the filter invocation is given in the right.}
\label{fig:tapes}
\end{figure}
...
Let $n$ be the number of initial values are provided to the feedback
joiner before values from the feedback loop are read.  Assume that the
round robin feedback joiner first draws from the data stream $I1_J$,
and then from the feedback loop $I2_J$.
