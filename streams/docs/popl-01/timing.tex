...
filter preceding $b$.  We now turn to deriving expressions for
...
the feedback loop $I2_J$.  The $min$ function for the main stream is
as before:
\begin{eqnarray*}
\mi{I1_J}{O_J}(x) = ceil(x/2) \\
\end{eqnarray*}
...
Equipped with definitions of $min$ and $max$ for tapes that are
connected by some series of downstream or
