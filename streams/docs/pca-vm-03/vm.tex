...
packets.  Because the elements are unordered, {\tt svm\_streamPeek}
and {\tt svm\_streamPeekEOS} cannot be used if this flag is set.  Note
that this implies that there is no EOS support for unordered
streams. \vspace{-2pt}\\}
...
buffer (as described above). \vspace{-2pt}\\}
...
than a circular buffer.\vspace{-2pt}\\}
...
of tracking a set of EOS tokens.\vspace{-2pt}\\}

\decl{\} svm\_Stream\_Flags;}
\makeline
%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage ~ \\ \vspace{-24pt}
\makeline
\name{streamPush}
...
\req{{\tt s} has been initialized and has a non-zero capacity.}
\callkc
%%%%%%%%%%%%%%%%%%%%%%%%%
\makeline
...
\req{{\tt s1, s2, ..., sn} have been initialized and have non-zero capacities.}
...
\req{
  ...
  {\tt s} does not have the {\tt svm\_Stream\_Unordered} flag set.\\
  {\tt s} has been initialized and has a non-zero capacity.}
...
\req{
  {\tt b} has been initialized and has a non-zero capacity.\\
...
\req{
  ...
  {\tt srcStr} and {\tt destStr} have been initialized and have non-zero capacities.\\
...
\req{
  ...
  {\tt srcStr} and {\tt destBlock} have been initialized and have non-zero capacities.\\
...
\req{
  ...
  {\tt srcBlock} and {\tt destStr} have been initialized and have non-zero capacities.\\
...
\req{
  {\tt scatter} is {\tt Unstarted}, {\tt Finished}, or uninitialized.\\
  {\tt srcStr}, {\tt indexStr}, and {\tt destBlock} have been initialized and have non-zero capacities.\\
  {\tt srcStr} and {\tt destBlock} have the same {\tt elementSize}.\\
...
\req{
  {\tt gather} is {\tt Unstarted}, {\tt Finished}, or uninitialized.\\
  {\tt srcBlock}, {\tt indexStr} and {\tt destStr} have been initialized and have non-zero capacities.\\
  {\tt srcBlock} and {\tt destStr} have the same {\tt elementSize}.\\
  {\tt indexStr} uses an unsigned representation for the indices.\\
  All accesses to {\tt srcBlock} are within its capacity.}
