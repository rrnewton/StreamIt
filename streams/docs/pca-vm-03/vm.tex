...
\item {\bf The VM is object-oriented.}  Kernels, streams, and stream graphs
are all represented as objects.  Among other benefits, this makes
explicit which data are local to kernels.
...
\ssss{Scatter/Gather}  The {\tt Scatter} and {\tt Gather} kernels are very similar to the
...
\ssss{Send}  The {\tt Send} kernel sends a stream from one processor to another,
...
\ssss{Receive}  The {\tt Receive} kernel receives a stream from a neighboring
...
\ssss{Send/Receive Protocol} We refer to channel number $n$ of connection $c$ as the pair $(c, n)$.
