...
%%     for (iteration=0; iteration<move->length || move->length == svm_Stream_Length_All; iteration++) {
%%       boolean eos = svm_streamPeekEOS(move->srcStr, 0);
%%       svm_streamPop(move->srcStr, buffer);
%%       if (!eos) {
%% 	// push normal item
%%         svm_streamPush(move->destStr, buffer);
%%       } else {
%% 	// push EOS item
%%         svm_streamPushEOS(move->destStr, buffer);
...
%%     kernelInit(&scatter->kernel, location, 0, move, sizeof(StridedScatter), (ExtKernelWork)&stridedScatterWork);
%%     move->srcStr = _srcStr;
%%     move->destBlock = _destBlock;
%%     move->length = _length;
%%     move->destStride = _destStride;
%%     move->elementsPerStride = _elementsPerStride;
%%   }

%%   void stridedScatterWork(StridedScatter* scatter) {
%%     int iteration, j;
%%     void* buffer = malloc(elementSize);
%%     for (iteration=0; iteration<move->length || move->length == svm_Stream_Length_All; iteration++) {
%%       boolean eos = svm_streamPeekEOS(move->srcStr, 0);
