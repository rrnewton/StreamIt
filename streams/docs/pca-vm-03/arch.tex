...
\item {\bf Architecture description is a graph rather than a tree,} to
support architectures such as Raw.

\item {\bf Memory nodes are simplified.}  There is only one type of
memory, with much fewer properties.
...
\enttable{
  ...
  \entry{VM\_PROP\_PROC\_SIMD} {int32 - SIMD/Vector speedup factor.}
    ...
    specified by the ILP property) if the input is data parallel.}
  ...
  \entry{VM\_PROP\_PROC\_IMEM} {int32 - Number of RISC ops} {The size
...
\enttable{
  ...
  \entry{VM\_PROP\_CHAN\_WINDOW\_SIZE} {int32 - Number of words} {The
  maximum number of words that can be in flight before the sender
  blocks.}

  \entry{VM\_PROP\_CHAN\_VISIBLE\_RANGE} {int32 - Number of words}
