...
$\mathbf{A'}$ is a $u' \times e'$ matrix and $\mathbf{{\vec b}'}$ is a
...
\mathbf{{\vec b}'} & = & b_1^e A_2^e + b_2^e \\ ~ \vspace{-8pt} \\
...
\mt{vTot} & = & \mt{vSum}_{n-1} \\ ~ \vspace{-4pt} \\
...
\multicolumn{3}{l}{\parbox{3in}{
	...
	\begin{array}{l}
	  1 ~\mt{if}~ i=j ~\wedge~ \\ 
	  ~\hspace{0.13in}~ vSum_{k} < i < vSum_{k+1} \\
	  0 ~\mt{otherwise}
	\end{array}
	\right.
      \end{array}
