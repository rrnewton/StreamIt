...
%
\vec{\mathbf{b}}'[j] & = & {\vec b}[u-1-(u'-j-1)~\mt{mod}~u]
%%
...
compiler detects performance-degrading combinations by use of the
automatic partitioning algorithm in Section~\ref{sec:partitioning}. 

\begin{figure}[t]
\center
\epsfxsize=3.2in
\epsfbox{images/example-pipeline-combination.eps}
\vspace{-6pt}
...
the order of the columns of $A$ generates the correct output value order.
Figure~\ref{fig:splitjoin-duplicate-matrix} elucidates the column ordering.
%though its notation is somewhat cumbersome.  
The equation can be understood as follows: for the $k$th child and 
the $m$th cycle of the joiner, the $n$th item that is pushed by child $k$ will
...
and ${\vec b}'$--define a linear node that exactly represents
the parallel combination of child nodes that are fed by a duplicate
splitter. Figure~\ref{fig:example-splitjoin-combination} illustrates
