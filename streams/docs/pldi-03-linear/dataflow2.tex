...
form if either of the terms is a known constant (eg $\vec{v}=\vec{0}$).
For division, the result is linear only if the divisor is a non-zero
...
is zero, since certain runtime inputs might cause a
