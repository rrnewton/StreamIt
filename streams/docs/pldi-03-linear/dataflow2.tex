...
\subsection{Linear Extraction Algorithm}
\label{sec:dataflow}
...
nodes that generate new linear forms.  A constant assignment $y = c$
...
Next are the instructions which combine linear forms.
...
\STATE $\tup{\vec v_2}{c_2} \leftarrow \mt{map}.\mt{get}(y_2)$
\STATE $\tup{\vec v_3}{c_3} \leftarrow \mt{map}.\mt{get}(y_3)$
\IF {$\vec v_3 = \vec 0_{Peek} \wedge y_3 \ne 0$}
