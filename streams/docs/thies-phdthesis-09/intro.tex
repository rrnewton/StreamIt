...
%% Our focus in the current work is on a new domain of programs which
%% we define as {\it stream programs}.  A stream program is one that
%% is organized around a regular flow of data, as in image, video, or
%% digital signal processing applications (see
%% Figure~\ref{fig:stream-graph}).  Examples include.... Our interest
%% in stream programs is motivated by two prominent trends in computer
...
\myitem {Computer architectures are becoming multicore.}  Because
...
\myitem {Computer applications are becoming embedded and
...
\myitem {Large streams of data.}  Perhaps the most fundamental
...
\myitem {Independent stream filters.}  Conceptually, a streaming
...
\myitem {A stable computation pattern.}  The structure of the stream
...
\myitem {Sliding window computations.}  Each value in a data stream
...
\myitem {Occasional modification of stream structure.}  Even though
...
\myitem {Occasional out-of-stream communication.}  In addition to
...
\myitem {High performance expectations.}  Often there are real-time
...
\myitem {Kahn Process Networks}, also known as {\it process
...
\myitem {Synchronous Dataflow (SDF)} is a restricted form of process
...
\myitem {Communicating Sequential Processes (CSP)} is in some ways
...
%% Brook~\cite{brook04}.  Our work on the StreamIt project was
...
internal use.
% TODO: Fix order?
Contributors to the benchmark suite include Sitij Agrawal, Basier
...
their own
work~\cite{mani-permutations,bit-streaming,ola-techrep,andersson_model_07,duca-thesis,won-thesis}.
...
\myitem {A design rationale and experience report for the StreamIt
...
\myitem {A new language construct, termed teleport messaging, that
...
\myitem {A review of the key results in optimizing StreamIt,
...
\myitem {The first translation of stream programs into the
...
\myitem {The first dynamic analysis tool that detects and exploits
