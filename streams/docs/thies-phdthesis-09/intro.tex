...
underlying machine.  In order to make headway against this
long-standing problem, it will likely be necessary to empower the
compiler with new information that is not currently embedded in
...
StreamIt, Chapter~3 also provides a survey of others' experience in
using and optimizing the language.

The specific contributions of this dissertation are as follows:

\begin{enumerate}

\item {\it A design rationale for the StreamIt language, which
  contains novel constructs to simultaneously improve the
  programmability and analyzability of stream programs (Chapter~2).}
  StreamIt is the first language to introduce hierarchical and
  structured streams, as well as a construct (termed {\it teleport
    messaging}) for timing the delivery of irregular control messages
  relative to regular flows of data in the program.  Other novelties
  include built-in language support for composable data reordering as
  well as sliding window operations, both of which expose key
  information to the compiler.

\item {\it A review of the key results and experienced gained in using
  and optimizing StreamIt, spanning parallelization, domain-specific
  optimization, and programmability (Chapter~3)}.  This chapter
  validates key concepts of the StreamIt language by highlighting the
  gains in performance and programmability that have been achieved,
  largely due to the work of others in the StreamIt group.  We focus
  on automatic parallelization (providing an 11.6x speedup on a
  16-core machine), domain-specific optimization of linear
  computations (eliminating up to 85\% of the floating point
  operations), and cache optimizations (providing a 3.5x speedup on
  embedded processors).  We also describe the experience of
  application programmers in developing the StreamIt benchmark suite.

\item {\it The first translation of stream programs into the
  lossless-compressed domain (Chapter~4)}.  This domain-specific
...
\item {\it The first dynamic analysis tool that detects and exploits
  likely coarse-grained parallelism in C programs (Chapter~5).}  To
