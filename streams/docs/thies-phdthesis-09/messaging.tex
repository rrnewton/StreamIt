...
%   - most messages sent with latency 0
%   - in mosaic, messages used to control a data-dependent feedback
%     loop, but not very natural
% - extensions to messaging:
%   - named work functions (as message receivers) -- allow switching in modes
%   - allow mechanism for hierarchical nodes to receive messages
% - describe legal constrained scheduling algorithm

\chapter{Teleport Messaging}
...
\vspace{-4pt}
\caption{Example \figsdep\ calculation for stream graph in
...
\caption{Example stream graph.  Nodes are annotated with their
...
\vspace{-8pt}
\caption{Example of unsatisfiable message constraints.  Each node is
  annotated with its input and output rate.  Messages are shown by
  dotted arrows, drawn from sender to receiver with a given latency.
...
\caption{Stream graph of frequency hopping radio with control
...
\vspace{-10pt}
\caption{Throughput as a function of the number of workstations
in the cluster. 
