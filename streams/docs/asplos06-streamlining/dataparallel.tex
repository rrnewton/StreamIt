...
\State $fraction = 1.0$
\State Stream $parent \gets $ \Call{getParent}{$F$}
\State Stream $child \gets F$
\While {$parent \not= $ null}
	\If {$parent$ is splitjoin}
		\State $total$-$work \gets$ \\
\ \ \ \ \ \ \ \ \ \ \ \ \ \  \ \ \ \ \  $\sum_{c \in children(parent)}$ \Call{avgWorkPerFilter}{$c$}
		\State $my$-$work \gets $ \Call{avgWorkPerFilter}{$child$}
		\State $fraction = fraction * my$-$work / total$-$work$
	\EndIf
	\State $child \gets parent$
	\State $parent \gets $ \Call{getParent}{$parent$}
\EndWhile
\Statex $\triangleright$ Fiss F according to its weight in task-parallel unit
\State Fiss $F$ into \Call{ceil}{$fraction * N$} filters
\end{algorithmic}
\end{algorithm}
