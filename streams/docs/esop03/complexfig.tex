...
\begin{align}
\label{eq:read}
{\cal D}_{{READ}_{c, p}} = \{~(i,j,k)~|~0 \le i \le N-1 ~\wedge~ 0 \le j \le S(n_b) - 1 ~\wedge~ 0 \le k \le E(c, p) - 1\}
\end{align}
%
\end{itemize}
%
...
\mt{num}(n)-1]$, and for each $q \in [0, \lfloor
\frac{\mt{E(c, p)}}{\mt{Period(c)}} \rfloor]$:
