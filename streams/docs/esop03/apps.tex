...
propagation, common sub-expression elimination, and 
\clearpage \noindent dead code elimination can all be performed
throughout the internals of the nodes as if they were connected in a
single control flow graph.
...
the motion of code from the steady-state to initialization time if it
does not depend on any quantity that is changing during the
steady-state execution of a node.  Quantities that the compiler
detects to be constant during the execution of a work function can be
computed ahead of time and placed as initial items on a channel, with
the results being passed through a feedback loop for availability on
future executions.
