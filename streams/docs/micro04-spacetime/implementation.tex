...
%% As mentioned in Section \ref{sec:raw} an I/O port is multiplexed
%% between its streaming memory module, an input stream originating
%% off-chip, and an output stream whose final destination is off-chip.
%% Only one trace with an off-chip, application input stream can be
%% mapped to a tile.  The same is true for a trace that writes to an
%% off-chip, application output stream.  But a single tile can
%% accommodate both an off-chip, application input stream and output
%% stream.
