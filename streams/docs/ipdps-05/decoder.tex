\begin{figure*}[t]
  \centerline{\epsfig{file=decoder_with_code.eps,width=\textwidth}}
...
that are output by the parser is dependent on the numbers frames in
...
contrast to the motion compensation and inverse transformations
stages, which consist of filters whose I/O rates are statically
resolved at compile time, the picture reordering and color space
conversion are parameterized on, and have I/O rates that depend on
initialization time constants; namely the pixel resolution of the
pictures.
...
transformations and motion compensation.  The implementation was
carried out by one student programmer with no prior understanding of
MPEG. The development spanned eight weeks from
...
with an example of support for multiple chrominance formats.
