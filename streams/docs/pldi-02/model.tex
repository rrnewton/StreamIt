...
abstract syntax within the semantic algebra.
...
$input -> (x -> let i = MIN(j s.t. sum_{k=0}^{j-1}{w_i} \le i%W) in 
                           \cal{S}(input)(
(\mi{I_S}{Oi_S} \circ \mi{Oi_S}{Ii_J} \circ \mi{Ii_J}{O_J})(x)))$
...
%% output.push(f_s_1(input.peek(0) \dots input.peek(peek_s - 1)))
%% \dots
%% output.push(f_s_push(input.peek(0) \dots input.peek(peek_s - 1)))
...
%% output.push(f_s1_1(input.peek(pop_s1*floor((j/2)/push_s1)),
%%                    \dots
%%                    input.peek(pop_s1*floor((j/2)/push_s1))))
%% output.push(f_s2_1(input.peek(0) \dots input.peek(peek_s - 1)))
%% \dots
%% output.push(f_s_push(input.peek(0) \dots input.peek(peek_s - 1)))
