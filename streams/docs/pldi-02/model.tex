...
single transfer function, $min$, that is defined for a given stream
graph. In Section \ref{sec:notation} we provide a definition of $min$
along with some notation.  We then motivate the $min$ function in
\ref{sec:informationflow} by deriving a concept of relative time and a
meaning for StreamIt's messaging system.  Only then, in Section
\ref{sec:derivemin}, do we turn to deriving expressions for the $min$
...
Now, we are ready to give a definition of the $min$ function:
\begin{definition}
$\mi{a}{b}(x)$ is the minimum number of items that must appear on tape
$a$ given that there are $x$ items on tape $b$, where $b$ is
downstream of $a$.
\end{definition}
Thus, one can think of $min$ as an inter-filter data dependence
...
firing is atomic).  The $min$ function generalizes this dependence to
...
Above, the $min$ function is described in terms of data dependences.
...
We can use the $min$ function to give a precise meaning to the message
...
\subsection{Deriving the Min Function}
\label{sec:derivemin}
...
&(({\cal S}[s_{p(i)}])(\la~i_{local}~.~g(\mi{I_S}{Op(i)_S}(i_{local}))))(\mi{Ip(i)_J}{O_J}(i))
\end{align*}
