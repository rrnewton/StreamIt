...
where: {\bf push_{sc}} &= totalRounds * W \\
       ...
       w_n)) \\
       totalPop &= \sum_{i=1}^{n}(totalRounds * w_i *pop_i / push_{si}) \\
       shift_j(x) &= \mi{I_S}{I_{sj}}(g(i_{local})+ \\ &~~~~~(\mi{I_{sj}}{O_{sj}} \circ \mi{O_{sj}}{O_J})(j)-peek_{sj}) \\
...
\item {\bf Parallelizing stateless filters.}  If a filter has no state,
  then we can gain task parallelism by duplicating the filter $n$ times
