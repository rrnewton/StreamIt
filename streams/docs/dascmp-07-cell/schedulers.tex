...
\subsection{StreamIt Compiler Backend for Cell}
...
high-level stream IR which then undergoes a suite of optimizing transformations.
...
generate the appropriate code to set up the scheduler for execution.
...
We generate separate code for the PPE and the SPEs. In both cases, we generate
C code that utilizes the library and compile it with Cell's GCC. We place all
control code on the PPE and all filter init and work functions on the SPEs. 
We convert the 
initialization and steady state schedules into appropriate sequences of calls to
the library. Specifically, we set up filter description and layout parameters
specifying which filter is to be executed on which SPE, which input and output
buffers the filter reads from and writes to, and how many bytes the filter 
reads and writes in one execution. These parameters and then passed through a
library function call which handles the necessary data transfers and executes the
work function of the filter. The initialization schedule initializes any state
of a filter and possibly executes the work function for an appropriate number of
iterations to preload the buffers with enough input to allow for continuous
steady state execution later. The steady state schedule is wrapped in a loop and
is executed for a given number of iterations. A callback function is registered 
with a filter execution. Upon callback, the function sets up parameters for the
next filter to be executed and fires that execution.
Because StreamIt uses a Java-like syntax, for the most part, filter init and work
functions can be directly translated into C code. Initialization code is
separated from work function code and is placed in its own filter so that it can
be executed separately in the init schedule. Some annotations, including filter
names and filter item types are defined.
