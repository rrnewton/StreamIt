...
for applications with an irregular distribution of work and the cost
of data-reorganization associated with filter merging is prohibitive
as it creates large memory structures. Also, mapping the complex
communication patterns described by a stream graph to the simple
interconnect of a parallel processor may introduce additional
synchronization points not present in the original graph.
...
multiple computational nodes. Our slice scheduling phase employs novel
...
slices of the graph and the bins represent the computational nodes of the
...
In the next sections we will present an overview of the input to the
SpaceTime compiler, its slice execution model, and its relevant
passes.  We will use our FM Radio application to further illustrate
these concepts.  
