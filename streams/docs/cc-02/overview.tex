...
is a {\tt Pipeline}.  The FFT in Figure \ref{fig:fft} is an example of
a {\tt Pipeline} that appears in our software radio.  Like a {\tt
Filter}, a {\tt Pipeline} has an {\tt init} function that is called
upon its instantiation.  However, there is no {\tt work} function, and
all input and output channels are implicit; instead, the pipeline
behaves as the sequential composition of filters that are specified
with successive calls to {\tt add} from within {\tt init}.
...
\end{enumerate}
\vspace{-6pt}Similarly, there are three kinds of joiners:\vspace{-6pt}
\begin{enumerate}
...
\end{enumerate}
\vspace{-6pt}The splitter and joiner type are specified with calls to {\tt
...
StreamIt supports filters with dynamically changing rates, including:\vspace{-6pt}
\begin{enumerate}
...
\vspace{-6pt}The last control construct provides a way to create cycles in the
...
in this paper (see \cite{streamittech} for a formal semantics), but
