...
num(n, \mt{init})-1]$, introduce this equation {\bf(IREAD to IWRITE)}:
%
\begin{align}
...
%
\item For each $c \in \mt{chan\_out}(n)$, and for each $p \in [0,
num(n, \mt{steady})-1]$, introduce this equation {\bf(SREAD to SWRITE)}:
%
\begin{align}
...
introduce this equation {\bf(SWRITE to SBUF)}:
\begin{align}
...
[0, \mt{num}(n, \mt{init})-1]$, introduce {\bf(IBUF to IREAD)}:
\begin{align}
...
}{\mt{Period(c)}} \rfloor ]$, introduce {\bf(SBUF to IREAD)}:
\begin{align}
...
\mt{num}(n, \mt{steady})-1]$, introduce this equation {\bf(IBUF to
SREAD)}:
\begin{align}
...
these equations {\bf(SBUF to SREAD)}:
