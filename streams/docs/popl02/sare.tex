...
%
\begin{align*}
\forall i \in {\cal D}_{{IWRITE}_{c, p}},~\mt{IWRITE}_{c,p}(i) = W(n, \mt{init}, p)(\mt{Input\_Arrays})[\mt{pos\_out}(n, c)][i] \\
\mt{where Input\_Arrays} = [\mt{IREAD}_{{chan\_in}(n)[1], p}(*), \dots, \mt{IREAD}_{{chan\_in}(n)[\mt{num\_in}(n)], p}(*) ]
\end{align*}
%
\item For each $c \in \mt{chan\_out}(n)$, and for each $p \in [0,
...
%
\begin{align*}
\forall (i, j, k) \in {\cal D}_{{SWRITE}_{c, p}},~\mt{SWRITE}_{c,p}(i,j,k) = W(n, \mt{steady}, p)(\mt{Input\_Arrays})[\mt{pos\_out}(n, c)][j] \\
\mt{where Input\_Arrays} = [\mt{SREAD}_{{chan\_in}(n)[1], p}(i, j, *), \dots, \mt{SREAD}_{{chan\_in}(n)[\mt{num\_in}(n)], p}(i, j, *) ]
\end{align*}
%
...
\mt{and } \mt{Offset}_{SW {\small \rightarrow} SB}(q,n,c,r) = q*\mt{TotalWrite}(n,c,\mt{steady}) + \mt{PartialWrite}(c,\mt{steady},r))
\end{align*}
%
...
}{\mt{TotalRead(n,c,steady)}} \rfloor ]$, introduce this equation (SBUF to
IREAD):
\begin{align*}
...
\mt{IREAD}_{c,p}(i) = 
    \mt{SBUF}_{c}(q,
      \mt{PartialRead}(c, \mt{init}, p) + i - 
            |{\cal D}_{{IBUF}_{c}}| - q * \mt{TotalRead}(n,c,\mt{steady})) \\
\mt{where }{\cal D}_{SB {\small \rightarrow} IR}(c,p) = 
  \{ i | i \in {\cal D}_{{IREAD}_{c,p}} ~\wedge~ 
         \mt{PartialRead}(c, \mt{init}, p) + i \notin {\cal D}_{{IBUF}_{c}} \}
\end{align*}
%
