...
copy-shift can be 2X to 3X slower than modulation, optimizations that
...
each execution.  Because each execution starts writing and reading to
...
Figures~\ref{fig:buf-arm},~\ref{fig:buf-p3} and~\ref{fig:buf-itanium},
...
performs from 2X to 3X slower than modulation.  The following sections
...
control flow within a actor, or if the loops are too large to fully unroll,
...
and~\ref{fig:buf-itanium} illustrate that scalar replacement offers
