...
of items to the filter's output tape.  A filter  may also {\it peek} at
a given index  on its input tape without  consuming  the  item;  this
makes  it  simple  to  represent computation over a
sliding-window.   The push, pop, and peek rates are  declared as
part  of  the work  function,  thereby enabling  the compiler    to
construct a static schedule of filter executions. The
...
their children in either a {\it duplicate} or a {\it roundrobin}
...
\begin{figure}[t]
\begin{center}
 \psfig{figure=ssi.eps,width=3.4in}
...
Figure~\ref{fig:ssi-all} we show the code size for the steady states
from our benchmark suite, and in Figure~\ref{fig:ssi-single} we
show a representative breakdown of the code size per filter in a
steady state execution of a StreamIt implementation of FFT. The
figures provide evidence that while individual filters may have a
