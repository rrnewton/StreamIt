...
purpose architecture are near their  performance limits for  the
...
optimizations} that are geared toward concomitantly increasing the {\it
temporal locality} of the instruction and data caches. These
...
SDF is a popular  model~\cite{LM87-i} that  is well suited for
...
for streaming computation that
simultaneously considers instruction and data locality for
...
together two or more are actors to form a coarser grained kernel.
...
alternatives (for buffer management) and introduces a formal model or
...
namely the {\it peek} operation whereby an actor may inspect data
...
While fusion alone delivers significant performance gains, our
...
The two architecture also differ in terms of their memory system
...
optimizations, and Section~\ref{sec:fusion} describes the 
