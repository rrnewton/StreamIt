...
compiler ensures that the actor's code size is not
...
%% , $(ii)$ improve the instruction temporal locality
...
84\% speedup for our benchmark suite on the StrongARM, a
101\% speedup on the Pentium~3, and a 103\% speedup on the Itanium~2. 

Scaling gives a 145\% speedup over unoptimized StreamIt for our 
benchmark suite on the StrongARM, a 58\% speedup on the Pentium~3 and a 
57\% speedup on the Itanium~2.

While both scaling and fusion lead to modest performance gains, we
combine the two to deliver a substantial performance improvement. When we
do so, we can achieve a speedup of 241\% for our benchmark suite on a
StrongARM, a speedup of 146\% on a Pentium~3, and a speedup of 144\% on 
an Itanium~2 processor. An optimized buffer management strategy in
...
%further improve the performance of our benchmarks by
...
demonstrating speedup of up to 8830\% on StrongARM, up to 5720\% on Pentium~3 and up to 4780\% 
on Itanium~2.
\end{itemize}
