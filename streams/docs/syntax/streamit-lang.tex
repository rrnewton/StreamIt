...
\hbox{$Id: streamit-lang.tex,v 1.18 2003-05-13 20:49:59 dmaze Exp $}
}\fi}
...
may peek up to the number of items specified.  As a special case, if
there are no \lstinline|pop| calls at all in the function body (all
access to the input tape is by peeking), the number of items declared
as the pop rate will be automatically popped at the end of the work
function.  All of the rate expressions must be compile-time constant.
...
\subsection{Output Peeking}

StreamIt's current \lstinline|peek| operator allows straightforward
construction of FIR filters, but a \lstinline|feedbackloop| is
necessary for IIR filters.  A simple IIR filter might have a transfer
function of
...
Implementing causal filters like this requires an operator like
\lstinline|peek|, but that peeks at values on the output tape of a
filter.  Implementation of this operator is mostly waiting on a name.
If it were named \lstinline|peer|, the above filter could be
implemented as

\begin{lstlisting}{}
float->float filter OutputPeeking {
  work pop 1 push 1 peer 1 {
    push(pop() + 0.5 * peer(0));
  }
}
\end{lstlisting}
