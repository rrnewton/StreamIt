...
(Figure~\ref{fig:??}); a SplitJoin construct can be divided into a
hierarchical set of SplitJoins to enable a finer granularity of fusion
(Figure~\ref{fig:??}); neighboring splitters and joiners with matching
weights can be eliminated (Figure~\ref{fig:??}); and nested pipelines
can be flattened into one (Figure~\ref{fig:??}).
