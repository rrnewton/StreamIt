...
\caption{\protect\small Stream graph of the original BeamFormer, in
which the Filters are at a granularity that is natural for the
algorithm.  This stream graph is a condensed version of the BeamFormer
described in \cite{pca}, which has 48 parallel channels instead of 2.
\protect\label{fig:beam-orig}}
\vspace{36pt}
\psfig{figure=beam-graph-opt.eps,width=6in}
\caption{\protect\small Stream graph of the load-balanced BeamFormer.
In this case, $\mbox{FIRFilter}_3$ is the most computationally
demanding node, and the aim of the load balancing phase is to
distribute its work.  To do this, we first perform vertical fusion of
$\mbox{FIRFilter}_3$ with three of its neighbors so as to avoid
devoting an entire tile to a lighter Filter.  Then, the fused filter
is parallelized five ways in each branch of the SplitJoin via a
horizontal fission transformation.  Finally, the InputGenerate
$\mbox{FIRFilter}_1$ nodes are fused into a filter $F$ to free up
additional tiles.  After these transformations, the bottleneck is $F$
rather than $\mbox{FIRFilter}_3$ (see
Figure~\ref{fig:beam-blood}). \protect\label{fig:beam-opt}}
\end{figure}

\begin{figure}
  \begin{center}
    \psfig{figure=beam-blood-key.eps,width=3in} \\
    \subfigure[\vspace{-6pt} Original.\label{fig:beam-blood1}]{\psfig{figure=beam-blood-orig.eps,width=3in}}
    \hspace{0.3in}
    \subfigure[\vspace{-6pt} Load-balanced.\label{fig:beam-blood2}]{\psfig{figure=beam-blood-opt.eps,width=3in} \vspace{-6pt}}
	\vspace{-6pt}
    \caption{Execution traces for the (a) original and (b)
    load-balanced partitionings of the BeamFormer.  The $x$ axis
    denotes time, and the $y$ axis denotes the processor.  The dark
    bands indicate periods where processors are blocked waiting to
    receive an input or send an output.  Light regions indicate
    periods of useful work. \label{fig:beam-blood}}
	\vspace{-6pt}
\label{fig:beam-blood} \end{center}
\end{figure}
...
\vspace{-12pt}
\caption{\protect\small Fusion of a Duplicate SplitJoin construct.
\protect\label{fig:fuse-splitjoin1}}
\vspace{-6pt}
\end{figure}
...
\protect\label{fig:fuse-splitjoin2}}
\vspace{-6pt}
\end{figure}
...
BeamFormer (Figure~\ref{fig:beam-orig}) is highly unbalanced, as the
$\mbox{FIRFilter}_3$ nodes constitute most of the work in the stream
graph.  Figure~\ref{fig:beam-blood1} provides an execution trace of
the original partitioning and demonstrates that these two filters are
the bottleneck for the entire computation.  The StreamIt compiler
performs four vertical fusion transformations and two five-way
horizontal fission transformations to obtain the load-balanced stream
graph given in Figure~\ref{fig:beam-opt}.  As illustrated by the
execution trace in Figure~\ref{fig:beam-blood2}, the optimized graph
achieves approximately five times as much processor utilization as the
original, since the computationally intensive nodes have been split
and the lighter nodes have been combined. In the following sections,
we describe these transformations in more detail.
