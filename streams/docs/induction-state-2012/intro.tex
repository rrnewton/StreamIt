...
The existence of state in a filter inhibits data parallelism opportunities.
Consider the example StreamIt filter in Figure~\ref{fig:apt-pipeline}.
It is based on the AssignPictureType filter in the MPEG2 encoder.
The filter maintains state in the variable {\it frameno}, which is
incremented on each filter invocation.  Each iteration of the 
filter uses a value for {\it frameno} dependent on what the value of
{\it frameno} was in the previous iteration. As such, it is not possible 
...
Figure~\ref{fig:apt-pipeline} shows how the same NoiseSource filter can be 
rewritten to use the keyword.  The compiler can generate the value
for {\tt iter()} for each iteration of the NoiseSource filter independent
of previous iterations.  Accordingly, it is possible
to data parallelize the filter.  
