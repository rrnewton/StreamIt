...
\caption{\protect\small Stream graph of the original 12x4 \BeamFormer.
The 12x4 \BeamFormer has 12 channels and 4 beams; it is the largest
version that fits onto 64 tiles without filter fusion.
...
\BeamFormer.  Vertical fusion is applied to collapse each pipeline
...
{\tt work} and read on later invocations.  Also, our current
implementation assumes that the {\tt initWork} function of the filter
is empty, although this is not fundamental to the algorithm. Let us
consider such a filter $F$ with steady-state I/O rates of $peek$,
$pop$, and $push$, that is being parallelized into an $K$-way
splitjoin.  There are two cases to consider:
\begin{enumerate}
