...
message constraints is an interesting problem.  In the case where
there are no overlapping constraints (i.e., two sender/receiver pairs
that span overlapping intervals of the stream graph), a simple
modification to pull scheduling suffices to satisfy the constraints.
First, note that a pull schedule will always satisfy constraints
imposed by upstream messages; because upstream (receiving) actors
execute as little as possible per execution of the downstream
...
As described in~\ref{sec:evaluation}, our compiler uses a simple
implementation of messaging in which each sender or receiver executes
in its own thread and waits for possible messages at appropriate
iterations.  This approach eludes the need to produce a serial
ordering of the actors at compile time.
