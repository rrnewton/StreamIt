...
This section defines a stream dependence function, $\sdep$, that can
be used to describe the timing of teleport messages.  The dependence
is meaningful for exactly those pairs of actors that are connected by
a directed path (aligned with the direction of data flow) in the
stream graph.  The {\it upstream} actor is at the start of the path,
while the {\it downstream} actor is at the end.  Dependences between
parallel actors (e.g., parallel branches of a splitjoin) currently
fall outside the scope of this model but could be addressed in future
...
is meaningful only if $A$ is upstream of $B$; otherwise, $\sdep$
assumes a value of zero.  Because the I/O rates of each actor in the
stream graph are known at compile time, $\sdep$ is also a static
relation.  The following is a formal definition of $\sdep$ using the
...
admit multiple pull schedules, as actors with multiple input channels
can traverse them in any order; however, the set of actor executions
remains constant even as the order changes.  The following theorem
...
A related fact is that $\sdep$ is not necessarily compositional.  For
...
$\sdepf{A}{B}(0) = 0$ by definition.  However, $\sdep$ does exhibit
compositionality within sequential regions (pipelines), and this
property can be exploited to tabulate $\sdep$ hierarchically instead
of via a complete pull schedule.
