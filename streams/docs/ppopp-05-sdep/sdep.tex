...
steady state $|{\cal S} \wedge X|$ when building the base $\sdep$
table:
...
\vspace{-2pt}
\end{equation*}
By growing the base $\sdep$ table by one steady state, this
formulation can set $q(n)$ to be one unit smaller than $p(n)$.  The
result is that the last complete steady state cycle is counted as part
...
desired.  Moreover, in complex cases\footnote{{\small For example, if
within each steady state, the first firing of $X$ does not depend on
the first firing of $Y$, and the last firing of $X$ does not depend on
the last firing of $Y$.}}, including the last complete steady state as
