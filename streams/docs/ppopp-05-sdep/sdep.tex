...
is an execution that does not change the buffering in the channels;
the number of items on each channel after the execution is the same as
it was before the execution.  Figure~\ref{fig:sdep1} simulates a
single steady state, meaning that additional entries of the pull
schedule will repeat the pattern given in the figure.  Thus, $\sdep$
will also grow in the same pattern, and we can calculate $\sdep$ for
$n > 4$ as follows\footnote{Note that for any two actors $X$ and $Y$,
$\sdepf{X}{Y}(0) = 0$}:
\begin{equation*}
\begin{array}{c}
\mbox{} \hspace{-5pt} \sdepf{A}{E}(n) = {\cal S}(A) * \floor{n-1}{{\cal S}(E)} + \sdepf{A}{E}(n-1~\mt{mod}~{\cal S}(E)) \vspace{3pt} \\
\mbox{} \hspace{-5pt} \sdepf{B}{E}(n) = {\cal S}(B) * \floor{n-1}{{\cal S}(E)} + \sdepf{B}{E}(n-1~\mt{mod}~{\cal S}(E))
\end{array}
\end{equation*}
where ${\cal S}(X)$ represents the number of times that actor $X$
appears in the steady state schedule.  In this example, ${\cal S}(A) =
6$, ${\cal S}(B) = 3$, and ${\cal S}(E) = 4$.
...
Instead, one can simulate ${\cal S}(Y)$ iterations as a pre-processing
step and answer all future $\sdep$ queries in constant time.  In
addition, the pull schedule for $Y$ can be reused to calculate $\sdep$
from $Y$ to any other actor ({\it e.g.,} $\sdepf{W}{Y}$ in addition to
$\sdepf{X}{Y}$).  
...
essential to calculate ${\bf pullSchedule}(Y, {\cal S}(Y))$, that is,
a steady state cycle of the pull schedule with respect to $Y$.
