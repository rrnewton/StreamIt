...
public class BasicGraphUI extends GraphUI implements Serializable {
     ...
     * live-preview in dragEnabled mode on Java 1.4.1 to workaround a bug that
    ...
    public class GraphTransferHandler extends TransferHandler {
        ...
        protected final Transferable createTransferable(JComponent c) {
            if (c instanceof JGraph) {
                ...
                if (cells != null && cells.length > 0) {
                    Object[] flat = graphLayoutCache.order(graph.getDescendantList(cells));
                    ParentMap pm = ParentMap.create(graphModel, flat, false, true);
                    ConnectionSet cs =
