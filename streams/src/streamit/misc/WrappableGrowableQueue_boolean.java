...
public class WrappableGrowableQueue_boolean implements WrappableGrowableQueue {
    ...
    public WrappableGrowableQueue_boolean() {
        sizeof_buffer = 2;
        buffer = new boolean[2];
        head = 0;
        tail = 0;
        sizeof_queue = 0;
    }

    public void enqueue(boolean i) {
    ...
    public boolean dequeue() {
        if (sizeof_queue == 0) {
            ...
            boolean return_val = buffer[tail];
    ...
    private void grow(int size) {
        ...
        boolean[] new_buffer = new boolean[size];
        int new_head = 0;
        int new_tail = 0;
