...
/**
 * Traverse a front-end tree and produce Java code. This uses
 * {@link streamit.frontend.nodes.FEVisitor} directly, without going through an
 * intermediate class such as <code>FEReplacer</code>. Every method actually
 * returns a String.
 * 
...
public class NodesToJava implements FEVisitor {
    ...
    public NodesToJava(boolean libraryFormat, boolean countops,
            TempVarGen varGen) {
        this.ss = null;
        this.indent = "";
    ...
    public String convertTypeFull(Type type, boolean includePrimitive) {
        if (type instanceof TypeArray) {
            ...
            return output + "[" + (String) array.getLength().accept(
                    this) + "]" + convertTypeFull(
                    array.getBase(),
                    false);
    ...
    public String doParams(List<Parameter> params, String prefix) {
        String result = "(";
        boolean first = true;       
    ...
    public String doAssignment(Expression lhs, Expression rhs,
        ...
        if (lhsType.isComplex()) {
            ...
            if (rhs instanceof ExprComplex) {
                ...
                return real.accept(this) + " = " + cplx.getReal().accept(
                        this) + ";\n" + imag.accept(this) + " = "
                        + cplx.getImag().accept(
                                this);
            } else
                return real.accept(this) + " = " + rhs.accept(this) + ";\n"
            ...
            if (rhs instanceof ExprComposite) {
                ...
                String result = x.accept(this) + " = " + cpst.getX().accept(
                        this) + ";\n" + y.accept(this) + " = "
                        + cpst.getY().accept(
                                this);
                Expression z1 = cpst.getZ();
                if (z1 != null)
                    result += ";\n" + z.accept(this) + " = " + z1.accept(this);
                Expression w1 = cpst.getW();
                if (w1 != null)
                    result += ";\n" + w.accept(this) + " = " + w1.accept(this);
                return result;
            } else
    ...
    public Object visitExprArray(ExprArray exp) {
        ...
        result += (String) exp.getBase().accept(
                this);
        result += "[";
        result += (String) exp.getOffset().accept(
                this);
        result += "]";
        return result;
    }
    ...
    public Object visitExprArrayInit(ExprArrayInit exp) {
        ...
        for (Expression e : elems) {      
            sb.append((String) e.accept(this));
            if (i != elems.size() - 1) {
                sb.append(",");
            }
    ...
    public Object visitExprBinary(ExprBinary exp) {
        ...
        String op = null;
        result = "(";
        result += (String) exp.getLeft().accept(
                this);
        switch (exp.getOp()) {
        ...
        result += " " + op + " ";
        result += (String) exp.getRight().accept(
                this);
        result += ")";

        // if profiling on, wrap this with instrumentation call
        if (libraryFormat && countops) {
            return wrapWithProfiling(
                    result,
                    binaryOpToProfilerId(exp.getOp()));
        }
    ...
    /**
     * Provides a mapping from binary operations in ExprBinary to the field name
    ...
    /**
     * Provides a mapping from unary operations in ExprUnary to the field name
     * in the Java library profiler (streamit.library.Profiler).
     */
    ...
    /**
     * Provides a mapping from java.Math functions to the field name in the Java
     * library profiler (streamit.library.Profiler).
     */
    ...
    /**
     * Given the generated code for an expression (expr) and the operation being
     * performed (op), wrap that expression in a call to an instrumentation
    ...
    private String wrapWithProfiling(String exp, String profilerId) {
        ...
        return "Profiler.registerOp(Profiler." + profilerId + ", " + id
                + ", \"" + exp.replace(
                        '\"',
                        '\'') + "\", " + exp + ")";
    }
    ...
    public Object visitExprComplex(ExprComplex exp) {
        ...
        String i = "";
        if (exp.getReal() != null)
            r = (String) exp.getReal().accept(
                    this);
        if (exp.getImag() != null)
            i = (String) exp.getImag().accept(
                    this);
        return "/* (" + r + ")+i(" + i + ") */";
    }
    ...
    public Object visitExprFunCall(ExprFunCall exp) {
        ...
        // else is a math function
            ...
            // take care of, e.g., init_array_1D_float(String filename, int
            // size)
            ...
            /*
             * The code below will insert the static array initializer in the
             * java output. For now we do this in the compiler instead to get
             * benefits of constant prop in the filename and size of the
             * initializer.
             * 
             * } else { result = makeArrayInit(exp); return result; }
             */
        ...
        for ( Object param : exp.getParams()) {
            if (!first)
                result += ", ";
            first = false;
            result += (String) ((Expression)param).accept(this);
        }
        result += ")";
        ...
        if (libraryFormat && countops && mathFunction) {
            result = wrapWithProfiling(
                    result,
                    funcToProfilerId(name));
        }
     ...
     * 
     * init_array_1D_float(String filename, int size) init_array_1D_int(String
     * filename, int size)
     * 
     * generates a static array initializer by loading the initial values from
     * the file.
     */
    ...
    private String makeArrayInit(ExprFunCall exp) {
        ...
        if (exp.getParams().get(
                0) instanceof ExprConstStr) {
            // for some reason the string literal has quotes on either
            // side
            String withQuotes = ((ExprConstStr) exp.getParams().get(
                    0)).getVal();
            filename = withQuotes.substring(
                    1,
                    withQuotes.length() - 1);
        } else {
            System.err.println("Error: expected first argument to " + funcName
        ...
        if (exp.getParams().get(
                1) instanceof ExprConstInt) {
            size = ((ExprConstInt) exp.getParams().get(
                    1)).getVal();
        } else {
            System.err.println("Error: expected second argument to " + funcName
                    + " to be an integer (the size)");
            System.exit(1);
        }
        ...
        if (funcName.equals("init_array_1D_int")) {
            int[] array = streamit.misc.Misc.init_array_1D_int(
                    filename,
                    size);
            // build result
            result.append("{");
        ...
        if (funcName.equals("init_array_1D_float")) {
            float[] array = streamit.misc.Misc.init_array_1D_float(
                    filename,
                    size);
            // build result
            result.append("{");
    ...
    public Object visitExprPeek(ExprPeek exp) {
        String result = (String) exp.getExpr().accept(
                this);
    ...
    public Object visitExprRange(ExprRange exp) {
        String min = (String) exp.getMin().accept(
                this);
        String ave = (String) exp.getAve().accept(
                this);
        String max = (String) exp.getMax().accept(
                this);
    ...
    public Object visitExprTernary(ExprTernary exp) {
        String a = (String) exp.getA().accept(
                this);
        String b = (String) exp.getB().accept(
                this);
        String c = (String) exp.getC().accept(
                this);
        switch (exp.getOp()) {
    ...
    public Object visitExprTypeCast(ExprTypeCast exp) {
        return "((" + convertType(exp.getType()) + ")("
                + (String) exp.getExpr().accept(
                        this) + "))";
    }

    @Override
    public Object visitExprUnary(ExprUnary exp) {
        String child = (String) exp.getExpr().accept(
                this);
        String result = null;
    ...
    public Object visitFunction(Function func) {
        ...
        if (ss == null) { // A helper function
            ...
            if (func.getCls() == Function.FUNC_NATIVE)
                ...
                result += " " + (String) func.getBody().accept(
                        this) + "\n";
            return result;
        }
    ...
    public Object visitProgram(Program prog) {
        ...
        for (TypeStruct struct : prog.getStructs()) {
            
            if (struct.getName().equals(
                    "String"))
                continue;
            if (libraryFormat && struct.getName().equals(
                    "float2"))
                continue;
            if (libraryFormat && struct.getName().equals(
                    "float3"))
    ...
    public Object visitSCSimple(SCSimple creator) {   
        ...
        boolean hardcoded_BitFileFlag = (creator.getName().equals(
                ...
                "FileWriter") || creator.getName().equals(
                "Writer"));

        String result;
        if (libraryFormat) {
            // Magic for builtins.
            if (creator.getName().equals(
                    "Identity") || creator.getName().equals(
                    "FileReader") || creator.getName().equals(
                    "FileWriter") || creator.getName().equals(
                    "ImageDisplay") || creator.getName().equals(
                    "Writer")) {
        ...
        for (Object type : creator.getTypes()) {
            if (!first) {
                result += ", ";
            }
            // Hacked to make FileReader/Writer<bit> work
    ...
    public Object visitSJRoundRobin(SJRoundRobin sj) {
        return "ROUND_ROBIN(" + (String) sj.getWeight().accept(
                this) + ")";
    }
    ...
    private Object doStreamCreator(String how, StreamCreator sc) {
        ...
        List<Expression> portals = sc.getPortals();
        if (portals.isEmpty()) {
        ...
        for (Object portal : portals) {
            result += ";\n" + indent + (String) ((Expression) portal).accept(this)
                    + ".regReceiver(" + tempVar + ")";
        }
        return result;
    }
    ...
    public Object visitStmtAssign(StmtAssign stmt) {
        ...
        String lhs = (String) stmt.getLHS().accept(
                this);
        String rhs = (String) stmt.getRHS().accept(
                this);

        // if profiling on and we are not just assigning, wrap rhs
        // with instrumentation call
        if (libraryFormat && countops && stmt.getOp() != 0) {
            rhs = wrapWithProfiling(
                    rhs,
                    binaryOpToProfilerId(stmt.getOp()));
        }
    ...
    public Object visitStmtDoWhile(StmtDoWhile stmt) {
        ...
        result += (String) stmt.getBody().accept(
                this);
        result += "while (" + (String) stmt.getCond().accept(
                this) + ")";
        return result;
    }
    ...
    public Object visitStmtEnqueue(StmtEnqueue stmt) {
        ...
        return "/* enqueue(" + (String) stmt.getValue().accept(
                this) + ") */";
    }

    @Override
    public Object visitStmtExpr(StmtExpr stmt) {
        String result = (String) stmt.getExpression().accept(
                this);
    ...
    public Object visitStmtIfThen(StmtIfThen stmt) {
        ...
        String result = "if (" + (String) stmt.getCond().accept(
                this) + ") ";
        result += (String) stmt.getCons().accept(
                this);
        if (stmt.getAlt() != null)
            result += " else " + (String) stmt.getAlt().accept(
                    this);
        return result;
    }
    ...
    public Object visitStmtJoin(StmtJoin stmt) {
        ...
        return "setJoiner(" + (String) stmt.getJoiner().accept(
                this) + ")";
    }
    ...
    public Object visitStmtReturn(StmtReturn stmt) {
        if (stmt.getValue() == null)
            return "return";
        return "return " + (String) stmt.getValue().accept(
                this);
    }

    @Override
    public Object visitStmtSendMessage(StmtSendMessage stmt) {
        String receiver = (String) stmt.getReceiver().accept(
                this);
        String result = "";
        ...
        if (stmt.getMinLatency() == null) {
            if (stmt.getMaxLatency() == null)
                ...
                result += receiver + ".setMaxLatency("
                        + (String) stmt.getMaxLatency().accept(
                                this) + ")";
        } else {
            ...
            result += receiver + ".setLatency("
                    + (String) stmt.getMinLatency().accept(
                            this) + ", " + (String) max.accept(this) + ")";
        }
        result += ";\n";
        if (libraryFormat) {
    ...
    public Object visitStmtSplit(StmtSplit stmt) {
        ...
        return "setSplitter(" + (String) stmt.getSplitter().accept(
                this) + ")";
    }
    ...
    public Object visitStmtWhile(StmtWhile stmt) {
        ...
        return "while (" + (String) stmt.getCond().accept(
                this) + ") " + (String) stmt.getBody().accept(
                this);
    }

    /**
     * For a non-anonymous StreamSpec, check to see if it has any message
     * handlers. If it does, then generate a Java interface containing the
     * handlers named (StreamName)Interface, and a portal class named
     * (StreamName)Portal.
     */
    private String maybeGeneratePortal(StreamSpec spec) {
        ...
        StringBuffer result = new StringBuffer();
        result.append(indent + "interface " + spec.getName() + "Interface {\n");
        addIndent();
    ...
    /**
     * For a non-anonymous StreamSpec in the library path, generate extra
     * functions we need to construct the object. In the compiler path, generate
    ...
    private String maybeGenerateConstruct(StreamSpec spec) {
        ...
        // (ASSERT: init != null)
        List<Parameter> params = init.getParams();
        ...
        if (spec.getType() == StreamSpec.STREAM_GLOBAL) {
            ...
            if (libraryFormat) {
                ...
                addIndent();
                result.append(indent
                        + "if (__instance == null) { __instance = new "
        ...
        if (libraryFormat) {
            ...
            for (Parameter param : params) {
                result.append(indent + "private "
                        + convertType(param.getType()) + " __param_"
                        + param.getName() + ";\n");
            }
            ...
            for (Object param  : params) {
                if (!first)
                    result.append(", ");
                first = false;
                result.append(convertType(((Parameter) param).getType()) + " "
            ...
            for (Object param  : params) {
                String name = ((Parameter) param).getName();
                result.append(indent + "__obj.__param_" + name + " = " + name
                        + ";\n");
            }
    ...
    public Object visitStreamSpec(StreamSpec spec) {
        ...
        if (spec.getName() != null) {
            ...
            assert false : "NodesToJava run before NameAnonymousStreams";
            /*
             * // Anonymous stream: result += "new "; switch (spec.getType()) {
             * case StreamSpec.STREAM_FILTER: result += "Filter"; break; case
             * StreamSpec.STREAM_PIPELINE: result += "Pipeline"; break; case
             * StreamSpec.STREAM_SPLITJOIN: result += "SplitJoin"; break; case
             * StreamSpec.STREAM_FEEDBACKLOOP: result += "FeedbackLoop"; break;
             * } result += "() {\n" + indent; addIndent();
             */
        }
        ...
        for (FieldDecl varDecl : spec.getVars()) {
    ...
    public Object visitTypeHelper(TypeHelper th) {
        ...
        if (libraryFormat && _native)
            return result;

        result += "class " + th.getName() + " extends "
                + (_native ? "NativeHelper" : "Helper") + " // "
                + th.getContext() + "\n";
        result += "{\n";
        ...
        if (node instanceof StmtIODecl) {
            ...
            if (ap.getPeek() == null) {
                ...
                if (ap.getPop() == null) {
                    ...
                    result += (String) ap.getPop().accept(
                            this) + ", ";
                }
            } else
                result += (String) ap.getPeek().accept(
                        this) + ", ";
            if (ap.getPop() == null)
                result += "0, ";
            else
            ...
            if (ap.getPush() == null)
                ...
                result += (String) ap.getPush().accept(
                        this) + ", ";
            result += "\"" + ap.getName() + "\")";
            return result;
        }
