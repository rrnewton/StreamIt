...
public class MagicDram extends IODevice {
    ...
    private void bcCode() {
        try {
            ...
            while (outfiles.hasNext()) {
                FileOutputContent out = (FileOutputContent) outfiles.next();
                fw.write(Util.getFileHandle(out) + " = fopen(\""
                         + out.getFileName() + "\", \"w\");\n");
            ...
            while (infiles.hasNext()) {
                FileInputContent in = (FileInputContent) infiles.next();
                fw.write(Util.getFileHandle(in) + " = fopen(\""
                         + in.getFileName() + "\", \"r\");\n");
                fw.write("if (" + Util.getFileHandle(in) + " == 0) {\n");
                fw.write("\tprintf(\"Error reading from file "
                         + in.getFileName() + "\");\n");
                fw.write("\treturn 0;\n");
                fw.write("}\n");
            }
            ...
            fw
                .write("\tprintf(\"// **** magic_dram: error adding device to port %d\\n\", port);\n");
            fw.write("\treturn 0;\n");
            fw.write("}\n");
            fw.write("\treturn 1;\n");
            fw.write("}\n\n");
            ...
            while (bufs.hasNext()) {
                ...
                fw.write("\tlocal " + bufferIdent + "_buffer = malloc("
                         + bufferIdent + "_size);\n");
            }
            ...
            // write the steady magic dram instructions with a while loop around
            // them
