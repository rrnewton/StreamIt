...
public class CodeStoreRenameAll extends SLIRReplacingVisitor {
	...
	/**
	 * This renamer is used by the renameOverAllFilters, to rename across
	 * filters, keep names distinct over all the filter
	 */
	...
	/**
	 * Make this private since it's dangerous to reuse the same renameall
	 * construct to rename across multiple filters
	 */
	...
	private void findDecls(JPhylum[] stmts) {
		for (int i = 0; i < stmts.length; i++) {
			...
			if (stmts[i] instanceof JVariableDeclarationStatement) {
				JVariableDeclarationStatement vds = (JVariableDeclarationStatement) stmts[i];
	...
	public Object visitFieldDeclaration(JFieldDeclaration self, int modifiers,
		...
		JVariableDefinition vardef = (JVariableDefinition) self.getVariable()
				.accept(this);
		return new JFieldDeclaration(self.getTokenReference(), vardef, null,
				null);
	}
	 ...
	 * String ident) {
	 * self.getVariable().setIdent(symtab.nameFor(self.getVariable
	...
	public Object visitForStatement(JForStatement self, JStatement init,
		...
		// System.err.println("switching symtab from " + symtab + " back to " +
		// ost);
		symtab = ost;
		return result;
	}
	...
	public Object visitNameExpression(JNameExpression self, JExpression prefix,
		...
		if (prefix != null)
			return new JNameExpression(self.getTokenReference(),
					(JExpression) prefix.accept(this), symtab.nameFor(ident));
		else
			return new JNameExpression(self.getTokenReference(), null,
	...
	public Object visitMethodCallExpression(JMethodCallExpression self,
		...
		JMethodCallExpression retval = new JMethodCallExpression(
				self.getTokenReference(), (JExpression) prefix.accept(this),
				classsymtab.nameFor(ident), newArgs);
		/*
		 * RMR { the method field may have been lost in which case preserve the
		 * tapeType which is a backup field carrying the function return type
		 */
	...
	public Object visitMethodDeclaration(JMethodDeclaration self,
		...
		JMethodDeclaration newdecl = new JMethodDeclaration(
				self.getTokenReference(), modifiers, returnType,
				symtab.nameFor(ident), newParams, exceptions,
				(JBlock) body.accept(this), null, null);
		// visit I/O rates
	...
	public Object visitFieldExpression(JFieldAccessExpression self,
		...
		JFieldAccessExpression fieldAccess = new JFieldAccessExpression(
				self.getTokenReference(), newLeft, newIdent, self.getField());
