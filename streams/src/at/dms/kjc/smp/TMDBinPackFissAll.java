...
public class TMDBinPackFissAll extends TMD {
    ...
    public void calculateFizzAmounts(int totalTiles) {
        ...
        for (Filter slice : slices) {
            ...
            int commRate = (fc.getPushInt() + fc.getPopInt()
                    * fc.getMult(SchedulingPhase.STEADY));
            ...
            if (StatelessFissioner.canFizz(
                    slice,
                    false)) {
                if (commRate > 0
                        && workEst / commRate <= FISS_COMP_COMM_THRESHOLD) {
                    System.out.println("Can fizz, but too much communication: "
                            + fsn);
                } else {
                    System.out.println("Can fizz: " + fsn + ", fizzing by: "
                            + totalTiles);
                    fizzAmount.put(
                            slice,
                            totalTiles);
    ...
    public void run(int tiles) {
        ...
        if (KjcOptions.dup == 1 || KjcOptions.optfile != null) {
            System.out
                    .println("***** Not using TMD scheduler since an SIR partitioner was used *****!");
            // multiply steady multiplicity of each filter by
            // KjcOptions.steadyMult
            LinkedList<Filter> slices = DataFlowOrder
                    .getTraversal(graphSchedule.getSSG().getTopFilters());

            for (Filter slice : slices) {
                WorkNodeContent filter = slice.getWorkNode()
                        .getWorkNodeContent();
        ...
        SMPBackend.scheduler.graphSchedule.getSSG().dumpGraph(
                "before_fission.dot",
                null,
                false);
            ...
            for (Filter slice : slices) {
                if (fizzAmount.containsKey(slice) && fizzAmount.get(slice) > 1) {
                    FissionGroup fissionGroup = StatelessFissioner.doit(
                            ...
                            fizzAmount.get(slice));

                    if (fissionGroup != null) {
                        // System.out.println("Fissing " + slice.getWorkNode() +
                        // " by " + fizzAmount.get(slice));
    ...
    public void runLayout() {
        ...
        assert graphSchedule != null : "Must set the graph schedule (multiplicities) before running layout";
    ...
    public SIRStream SIRFusion(SIRStream str, int tiles) {
        ...
        // bin pack the shits
        StatefulGreedyBinPacking binPacker = new StatefulGreedyBinPacking(str,
                tiles, work);
        binPacker.pack();
        ...
        do {
            ...
            int minTiles = at.dms.kjc.sir.lowering.partition.Partitioner
                    .estimateFuseAllResult(str);
            ...
            if (numStatefulFilters - 1 <= minTiles + 1) {
                ...
                System.out.println("  Attempting to fuse down to "
                        + (numStatefulFilters - 1) + " stateful filters");
                str = new StatefulFusion(str, work, numStatefulFilters - 1,
                        false).doFusion();
            }
            ...
            // if number of stateful filters didn't change, then quit since
            // can't fuse anymore
            int newNumStatefulFilters = StatefulFusion
                    .countStatefulFilters(str);
     ...
     * 
     * @param tiles
     *            The number of tiles we are targeting
    ...
    private int multiplicityFactor(int tiles) {
        ...
        for (Filter slice : slices) {
            ...
            if (fizzAmount.containsKey(slice) && fizzAmount.get(slice) > 1) {
                // this works only for pipelines right now, so just that we have
                // at most
                // one input and at most one output for the slice
                // assert
                // slice.getHead().getSourceSet(SchedulingPhase.STEADY).size()
                // <= 1;

                // check that we have reached the threshold for duplicated items
                int threshFactor = (int) Math
                        .ceil((((double) (fi.peek - fi.pop)) * fizzAmount
                                .get(slice))
                                / (DUP_THRESHOLD * (((double) fi.pop) * ((double) fi.steadyMult))));

                // this factor makes sure that copydown is less than
                // pop*mult*factor
                int cdFactor = (int) Math
                        .ceil(fi.copyDown
                                / ((double) (fi.pop * fi.steadyMult) / (double) (fizzAmount
                                        .get(slice))));

                int myFactor = Math.max(
                        cdFactor,
                        threshFactor);
