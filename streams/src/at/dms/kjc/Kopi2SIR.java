...
public class Kopi2SIR extends Utils implements AttributeVisitor<Object>, Cloneable {
	...
	/*
	 * Object used to disregard the return values from the attributed visitor
	 * methods
	 */
	...
	// List of lists of final vars encountered to allow propagation into
	// anonymous classes
	// Each element of finalVars represents the list of finalvars encountered
	// for a method
	...
	// for methods that have I/O rate declarations, maps method name to push,
	// pop, peek rate
	// String -> JExpression
	private HashMap<String, JExpression> methodToPushRate, methodToPopRate,
			methodToPeekRate;
	...
	// when they are added, they will be cloned and any parameters that need to
	// be set
	// will be set... (because of nesting of constructor calls,
	// initBuiltinFilters
	...
	private void addVisitedOp(String className, SIROperator sirop) {
		/*
		 * Don't worry about duplicates; this will be caught by the type checker
		 * of Kopi, since you can only have one Java class defined per package.
		 * In the case of recursive streams, adding an op a second time will
		 * obliterate the first one, although nothing should rely on this.
		 * 
	...
	private SIROperator getVisitedOp(String className) {
		...
		// if not look for it over the entire application (if given)
			...
			if (searchList.contains(className)) {
				...
				at.dms.util.Utils
						.fail("Mutually recursive stream defintion of "
								+ className);
			}
			return searchForOp(className);
		}
		at.dms.util.Utils.fail(lineNumber
				+ ": Cannot find declaration of stream " + className);
		return null;
	}

	/**
	 * Returns whether or not the given name represents a toplevel (void->void)
	 * stream that can be executed by itself.
	 */
	public boolean isToplevelStream(String name) {
		return (name.equals("StreamIt") || name.equals("StreamItFilter")
				|| name.equals("StreamItPipeline")
				|| name.equals("StreamItSplitJoin") || name
					.equals("StreamItFeedbackLoop"));
	}
	 ...
	 * </pre>
	 * 
	 * and returns associated op. Does not worry about anything
	 * recursive--should call getVisitedOp in the general case.
	 */
		...
		for (int unit = 0; unit < application.length; unit++) {
			...
			for (int i = 0; i < decls.length; i++) {
				if (decls[i] instanceof JClassDeclaration) {
					...
					// get the SIRStream
					if (((JClassDeclaration) decls[i]).getSourceClass()
							.getIdent().equals(className)) {
	...
	private void blockStart(String str, JPhylum getLineFromMe) {
		if (getLineFromMe != null && getLineFromMe.getTokenReference() != null)
			lineNumber = getLineFromMe.getTokenReference().getLine();
		printMe("attribute_visit" + str + "\n");
	}

	/**
	 * Returns true if there is a function of the given name in
	 * 
	 * <pre>
	 * clazz
	 * </pre>
	...
	/*
	 * creates a new SIROperator setting parentStream to this object also, if
	 * the SIROperator is one to one, parentStream is set
	 */
	private SIROperator newSIROP(JClassDeclaration clazz) {
		...
		if (TYPE.equals("Pipeline") || TYPE.equals("StreamIt")
				|| TYPE.equals("StreamItPipeline")) {
			SIRPipeline current = new SIRPipeline((SIRContainer) parentStream,
					clazz.getIdent(), JFieldDeclaration.EMPTY(),
		...
		if (TYPE.equals("SplitJoin") || TYPE.equals("StreamItSplitJoin")) {
			SIRSplitJoin current = new SIRSplitJoin(
					(SIRContainer) parentStream, clazz.getIdent());
			parentStream = current;
			return current;
		}

		at.dms.util.Utils.fail(printLine(clazz)
				+ "You are creating an unsupported StreamIt Operator: " + TYPE
				+ ".");
		return null;
	}

	/**
	 * Returns true if the given JStatement defines a call to an SIR expression
	 **/
	...
	/**
	 * Returns true if the JStatement contains a print or println
	 * 
	 * @param exp
	 * @return true if the Statement contains a print or println, false
	 *         otherwise.
	 */
	private boolean isPrintExpr(JMethodCallExpression exp) {
		if (exp.getIdent().equals("print") || exp.getIdent().equals("println")) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Returns whether str1 equals str2+suffix, where suffix is one of the types
	 * accepted by StreamIt.
	 */
	...
	/*
	 * Given a method call expression, this function will create/return a SIR
	 * Expression if the method call orignally denoted one.
	 */
	private JPhylum newSIRExp(JMethodCallExpression exp, JExpression[] args) {
		...
		if (matchesPrefix(exp.getIdent(), "push")) {
			...
			((SIRPushExpression) newExp).setTapeType(parentStream
					.getOutputType());
		}
		if (matchesPrefix(exp.getIdent(), "pop")) {
			newExp = new SIRPopExpression();
			((SIRPopExpression) newExp)
					.setTapeType(parentStream.getInputType());
		}
		if (matchesPrefix(exp.getIdent(), "peek")) {
			newExp = new SIRPeekExpression(args[0]);
			((SIRPeekExpression) newExp).setTapeType(parentStream
					.getInputType());
		}
	...
	private void postVisit(SIROperator current) {
		...
		if (current instanceof SIRStream) {
			...
			for (int i = 0; i < methods.length; i++) {
				...
				if (push != null)
					methods[i].setPush(push);
				if (peek != null)
					methods[i].setPeek(peek);
				if (pop != null)
					methods[i].setPop(pop);
			}
		}
	}
	...
	private boolean supportedType(String type) {
		if (type.equals("Integer") || type.equals("Character")
				...
				|| type.equals("Long") || type.equals("Bit"))
			return true;
		else
			return false;
	}
	...
	// if the given filter is a builtin filter (FILE i/o) set what needs to be
	// set
	private void setBuiltinArgs(SIROperator stream, JExpression[] args) {
		...
		if (stream instanceof SIRFileReader) {
			if (args.length != 2 && args.length != 3) // can be 3 args with Bit
														// hack
				at.dms.util.Utils.fail(lineNumber
						+ ": 2 or 3 args required for FileReader");
			...
			if (!(args[1] instanceof JStringLiteral))
				at.dms.util.Utils.fail(lineNumber
						+ ": Second argument to FileReader must be a type");
			((SIRFileReader) stream).setInputType(CStdType.Void);
			((SIRFileReader) stream)
					.setOutputType(getType(((JStringLiteral) args[1])
							.stringValue()));
			return;
			...
			if (args.length != 2 && args.length != 3) // can be 3 args with Bit
														// hack
				at.dms.util.Utils.fail(lineNumber
						+ ": 2 or 3 args required for FileWriter");
			((SIRFileWriter) stream).setFileName(args[0]);
			// the second arg will be turned into string by visitFieldExpression
			// because it is a type
			if (!(args[1] instanceof JStringLiteral))
				at.dms.util.Utils.fail(lineNumber
						+ ": Second argument to FileWriter must be a type");
			((SIRFileWriter) stream)
			...
			if (args.length == 1) {
				...
				if (args[1] instanceof JStringLiteral)
					((SIRIdentity) stream)
							.setType(getType(((JStringLiteral) args[1])
									.stringValue()));
				else if (args[1] instanceof JMethodCallExpression)
					((SIRIdentity) stream)
							.setType(((JMethodCallExpression) args[1])
									.getPrefix().getType());
				else if (args[1] instanceof JClassExpression)
					((SIRIdentity) stream).setType(((JClassExpression) args[1])
							.getClassType());
				else
	 ...
	 * not currently used
	 * 
	 * private JMethodDeclaration[] buildPortalMethodArray(JClassDeclaration
	 * portal, JClassDeclaration clazz) { JMethodDeclaration[] portalMethods =
	 * portal.getMethods(); JMethodDeclaration[] clazzMethods =
	 * clazz.getMethods(); JMethodDeclaration[] handlerMethods = new
	 * JMethodDeclaration[portalMethods.length];
	 * 
	 * //For each method declared in the portal class //find the corresponding
	 * method in the given clazz //build the handlerMethods as we go for (int i
	 * = 0; i < portalMethods.length; i++) { JMethodDeclaration handlerMethod =
	 * null; for (int j = 0; j < clazzMethods.length; j++) { //Check if the
	 * signature of the current method equals the portal method //this may not
	 * work, I will try it! if
	 * (portalMethods[i].getMethod().equals(clazzMethods[j].getMethod()))
	 * handlerMethod = clazzMethods[j]; } if (handlerMethod == null)
	 ...
	 * clazz.getSourceClass().getIdent()); handlerMethods[i] = handlerMethod; }
	...
	public Object visitClassDeclaration(JClassDeclaration self, int modifiers,
			...
			JMethodDeclaration[] methods, JTypeDeclaration[] decls) {
		 ...
		 * "symbol table" Adding it earlier to support not visit recursive def
		 * again
		 */
		...
		/*
		 * for(int i=0;i<spaces;i++) System.err.print("  ");
		 * System.err.println(spaces+" Starting Visiting: "+ident); spaces++;
		 */
		trash = visitClassBody(decls, fields, methods, body);
		/*
		 * spaces--; for(int i=0;i<spaces;i++) System.err.print("  ");
		...
		/*
		 * if (self.getSourceClass().getSuperClass().getIdent().equals(ident))
		 * anonCreation = true; else anonCreation = false;
		 */

		parentStream = oldParentStream;
		printMe("Out " + self.getSourceClass().getSuperClass().getIdent() + num);
	...
	public Object visitCompilationUnit(JCompilationUnit self,
		...
		try {
			...
			/*
			 * if (packageName.getName().length() > 0) packageName.accept(this);
			 */
			...
			for (int i = 0; i < typeDeclarations.length; i++) {
				if (typeDeclarations[i] instanceof JClassDeclaration) {
					JClassDeclaration decl = (JClassDeclaration) typeDeclarations[i];
					if (isToplevelStream(decl.getSourceClass().getSuperClass()
							.getIdent())) {
						if (TopLevelDeclaration != null)
							at.dms.util.Utils.fail(printLine(decl)
									+ "Top level stream already set.");
						TopLevelDeclaration = decl;
					} else if (decl.getSourceClass().getSuperClass().getIdent()
							.equals("Structure")) {
						...
						structureList.add(sirStruct);
							...
							.equals("Helper")
							|| decl.getSourceClass().getSuperClass().getIdent()
									.equals("NativeHelper")) {
						SIRHelper sirHelper;
	...
	public Object visitFieldDeclaration(JFieldDeclaration self, int modifiers,
		...
		/*
		 * Input declaration set the fields for the current stream
		 */
		/*
		...
		/*
		 * Output declaration set the fields for the current stream
		 */
		...
		parentStream.addField(new JFieldDeclaration(null,
				new JVariableDefinition(null, modifiers, type, ident, expr),
				null, null));
	...
	public Object visitMethodDeclaration(JMethodDeclaration self,
		...
		if (ident.equals("work")) {
			if (parentStream instanceof SIRFilter)
				((SIRFilter) parentStream).setWork(new JMethodDeclaration(null,
						modifiers, returnType, ident, parameters, exceptions,
						body, null, null));
			else
				at.dms.util.Utils.fail(printLine(self)
		...
		// install prework function
			...
			if (parentStream instanceof SIRTwoStageFilter) {
				((SIRTwoStageFilter) parentStream)
						.setInitWork(new JMethodDeclaration(null, modifiers,
								returnType, ident, parameters, exceptions,
								body, null, null));
			} else
				at.dms.util.Utils.fail(printLine(self)
		...
		/* Install init function for filter */
			...
			((SIRFeedbackLoop) parentStream)
					.setInitPath(new JMethodDeclaration(null, modifiers,
							returnType, ident, parameters, exceptions, body,
							null, null));
	...
	public Object visitWhileStatement(JWhileStatement self, JExpression cond,
		...
		return new JWhileStatement(self.getTokenReference(), newCond, newBody,
				self.getComments());
	}
		...
		for (int i = 0; i < vars.length; i++)
			if (vars[i] != null)
				return new JVariableDeclarationStatement(
						self.getTokenReference(), vars, self.getComments());
		return null;
	}

	/**
	 * visits a variable declaration statement. Instead of creating a new
	 * variable definition, we mutate the old one and return it so that all of
	...
	public Object visitVariableDefinition(JVariableDefinition self,
		...
		// The attribute returned from visiting the expression of the variable
		// def
		Object retObj = null;
		...
		if (retObj instanceof JUnqualifiedInstanceCreation) {
			...
			if (type.getCClass().getSuperClass().getIdent().equals("Structure")) {
				...
				return self;
				// return new JVariableDefinition(null, modifiers, type, ident,
				// null);
			}
			...
			// we need to add this definition to the symbol table
				...
				SIRStream ST = (SIRStream) ObjectDeepCloner.deepCopy(temp);
				printMe("Adding " + ident + " to symbol table");

				// If this a builtin filter set its args
				setBuiltinArgs(ST,
						((JUnqualifiedInstanceCreation) retObj).getParams());
				SIRInitStatement newSIRInit = new SIRInitStatement(
						Arrays.asList(((JUnqualifiedInstanceCreation) retObj)
								.getParams()), ST);
	...
	public Object visitSwitchStatement(JSwitchStatement self, JExpression expr,
		...
		return new JSwitchStatement(self.getTokenReference(), expr, body,
				self.getComments());
	}
	...
	public Object visitReturnStatement(JReturnStatement self, JExpression expr) {
		...
		if (expr != null)
			expr = (JExpression) expr.accept(this);
		return new JReturnStatement(self.getTokenReference(), expr,
				self.getComments());
	}
	...
	public Object visitLabeledStatement(JLabeledStatement self, String label,
		...
		return new JLabeledStatement(self.getTokenReference(), label, stmt,
				self.getComments());
	}
	...
	public Object visitIfStatement(JIfStatement self, JExpression cond,
		...
		return new JIfStatement(self.getTokenReference(), cond, thenClause,
				elseClause, self.getComments());
	}
	...
	public Object visitCompoundStatement(JCompoundStatement self,
		...
		return new JCompoundStatement(self.getTokenReference(), body);
	}
	...
	public Object visitExpressionStatement(JExpressionStatement self,
		...
		if (attrib instanceof JExpression)
			return new JExpressionStatement(self.getTokenReference(),
					(JExpression) attrib, self.getComments());
	...
	public Object visitExpressionListStatement(JExpressionListStatement self,
		...
		return new JExpressionListStatement(self.getTokenReference(), expr,
				self.getComments());
	}
	...
	public Object visitDoStatement(JDoStatement self, JExpression cond,
		...
		if (body != null)
			body = (JStatement) body.accept(this);
		return new JDoStatement(self.getTokenReference(), cond, body,
				self.getComments());
	}
	...
	public Object visitBlockStatement(JBlock self, JavaStyleComment[] comments) {
		...
		// nullVector.add(null);
		LinkedList<JStatement> bodyList = new LinkedList<JStatement>(
				Arrays.asList(body));
		...
		return new JBlock(self.getTokenReference(), newBody, comments);
	...
	public Object visitThisExpression(JThisExpression self, JExpression prefix) {
		...
		return self;
		/*
		 * if (prefix != null) { prefix = (JExpression)prefix.accept(this); }
	...
	public Object visitQualifiedInstanceCreation(
		...
		for (int i = 0; i < params.length; i++)
			params[i] = (JExpression) params[i].accept(this);
		return new JQualifiedInstanceCreation(self.getTokenReference(), prefix,
				ident, params);
	}
	...
	public Object visitUnqualifiedAnonymousCreation(
		...
		/*
		 * for (int i = 0; i < decl.fields.length; i++) { trash =
		 * decl.fields[i].variable.accept(this); }
		 */
		// The only time that we can use an Anonymous class is inside an add
		// statement
		// which needs to be translated into an SIRInitStatement
		// System.err.println("CONNING anon: "+SIROp);
		SIRInitStatement sis = new SIRInitStatement(Arrays.asList(params),
				(SIRStream) ObjectDeepCloner.deepCopy((SIRStream) SIROp));
		return sis;
	}
	...
	public Object visitUnqualifiedInstanceCreation(
		...
		return new JUnqualifiedInstanceCreation(self.getTokenReference(), type,
				params);
	}
	...
	public Object visitNewArrayExpression(JNewArrayExpression self, CType type,
		...
		return new JNewArrayExpression(self.getTokenReference(), type, dims,
				init);
	}
	...
	private SIRMessageStatement createMessageStatement(
			JMethodCallExpression methCall, CType portal, JExpression prefix,
			JExpression[] args) {
		// Process the args
		...
		CClassType[] interfaces = portal.getCClass().getInterfaces();
		 ...
		 * //index of the method in the portal interface int index = -1; for
		 * (int i = 0; i < portalMethods.length; i++) { if
		 * (portalMethods[i].getIdent().equals(methCall.getIdent())) { index =
		 * i; break; } } if (index == -1) //cannot find method
		 * at.dms.util.Utils.fail("Cannot find portal method " +
		 * methCall.getIdent() + " in portal interface");
		 */
		...
		/*
		 * debugging System.err.println(printLine(methCall) + " latency " +
		 * nextLatency.toString());
		 */
		return new SIRMessageStatement(prefix, interfaceName,
				methCall.getIdent(), args, nextLatency);
	}

	// given a method call, a JLocalVariableExpression portal, and a stream,
	// build the
	// SIRRegReceiveStatement
	private SIRRegReceiverStatement createRegReceiver(
			JMethodCallExpression methCall, CType portal, JExpression prefix,
			SIRStream st) {
		...
		// Build matchedMethods by iterating over all the methods in the portal
		// interface
		...
		for (int i = 0; i < portalMethods.length; i++) {
			...
			for (int j = 0; j < streamMethodDecs.length; j++) {
				if (portalMethods[i].getIdent().equals(
						streamMethodDecs[j].getName())) {
	...
	private SIRInitStatement createInitStatement(Object SIROp, String regMethod) {
		...
		/*
		 * //Already an SIRinit statement if (SIROp instanceof SIRInitStatement)
		 * { return (SIRInitStatement)SIROp; } //Using a local variable that has
		 * been already defined as a stream construct if (SIROp instanceof
		 * SIRStream) newST = (SIRStream)SIROp;
		 */

		// Already an SIRinit statement (this is a local variable expression or
		// a anonymous
		// creation)
		// we use the init statement from the symbol table (for a local var) or
		// created it in the
		...
		if (SIROp instanceof JUnqualifiedInstanceCreation) {
			...
			if (st == null)
				at.dms.util.Utils.fail(lineNumber
						...
						+ ((JUnqualifiedInstanceCreation) SIROp).getType()
								.getCClass().getIdent());
		...
		if (SIROp instanceof SIRInitStatement)
			...
			if (newST instanceof SIRIdentity)
				...
				return new SIRInitStatement(
						Arrays.asList(((JUnqualifiedInstanceCreation) SIROp)
								.getParams()), newST);
		}
	}
	...
	public Object visitMethodCallExpression(JMethodCallExpression self,
		...
		if (isSIRExp(self)) {
			...
			assert parentStream instanceof SIRFilter : "setStateful should only be called for filters";
			...
			/*
			 * debugging
			 * System.err.println("nextLatency = SIRLatency.BEST_EFFORT");
			 */
			return null;
			...
			/*
			 * debugging
			 * System.err.println("nextLatency = SIRLatencyMax("+args[0]+")");
			 */
			return null;
			...
			/*
			 * debugging
			 * System.err.println("nextLatency = SIRLatencyRange("+args
			 * [0]+","+args[1]+")");
			 */
			...
			// we want to ignore remove this method from the block
			return null;
		} else if (ident.equals("addSteadyPhase")
				|| ident.equals("addInitPhase")
				|| ident.equals("annotateIORate")) {
			...
			if (parentStream instanceof SIRFilter) {
				...
				if (args[0] instanceof JClassExpression)
					filter.setInputType(((JClassExpression) args[0])
							.getClassType());
				else if (args[0] instanceof JStringLiteral)
					filter.setInputType(getType(((JStringLiteral) args[0])
							.stringValue()));
				else if (args[0] instanceof JMethodCallExpression) {
					...
					if (methCall.getIdent().equals("getClass")) {
						...
						if (methCall.getPrefix().getType() instanceof CArrayType)
							((CArrayType) methCall.getPrefix().getType())
									.setDims(((JNewArrayExpression) methCall
											.getPrefix()).getDims());
					...
					if (t.getType().toString().equals("java.lang.Void")) {
						...
						Utils.fail(printLine(self)
								+ "Malformed input type on I/O declaration: "
								+ args[0] + " " + t.getType());
					}
				} else
					Utils.fail(printLine(self)
				...
				if (args[1] instanceof JClassExpression)
					filter.setOutputType(((JClassExpression) args[1])
							.getClassType());
				else if (args[1] instanceof JStringLiteral)
					filter.setOutputType(getType(((JStringLiteral) args[1])
							.stringValue()));
				else if (args[1] instanceof JMethodCallExpression) {
					// defining an array to be passed over the tape...
					...
					if (t.getType().toString().equals("java.lang.Void")) {
						...
						Utils.fail(printLine(self)
								+ "Malformed output type on I/O declaration: "
								+ args[1] + " " + t.getType());
					}
				} else
					Utils.fail(printLine(self)
		...
		// I think that this is the only case when prefix will not be null???
		// Keep the check any way
		else if (prefix instanceof JLocalVariableExpression
				&& ((JLocalVariableExpression) prefix).getVariable().getType()
						.toString().endsWith("Portal")) {
		...
		// message send to portal using field as portal
				...
				&& ((JFieldAccessExpression) prefix).getType().toString()
						.endsWith("Portal")) {
			if (ident.equals("regSender"))
				return null;
			return createMessageStatement(self,
					((JFieldAccessExpression) prefix).getType(), prefix, args);
	...
	private void buildSplitter(JExpression type) {
		...
		// after we have visited the entire stream, we will call setSplitter
		// again
		...
		if (splitterType.getIdent().equals("ROUND_ROBIN")) {
			...
			JExpression weight = (weightArgs.length > 0 ? weightArgs[0]
					: new JIntLiteral(1));
			splitter = SIRSplitter.createUniformRR((SIRContainer) parentStream,
					weight);
		} else if (splitterType.getIdent().equals("DUPLICATE"))
			splitter = SIRSplitter.create((SIRContainer) parentStream,
			...
			splitter = SIRSplitter.createWeightedRR(
					(SIRContainer) parentStream, splitterType.getArgs());
		else
			at.dms.util.Utils.fail(printLine(type) + "Unsupported Split Type");
	...
	private void buildJoiner(JExpression type) {
		...
		if (joiner.getIdent().equals("ROUND_ROBIN")) {
			...
			JExpression weight = (weightArgs.length > 0 ? weightArgs[0]
					: new JIntLiteral(1));
			joinType = SIRJoiner.createUniformRR((SIRContainer) parentStream,
					weight);
		} else if (joiner.getIdent().equals("COMBINE"))
			joinType = SIRJoiner.create((SIRContainer) parentStream,
					SIRJoinType.COMBINE, 2);
		else if (joiner.getIdent().equals("NULL"))
			joinType = SIRJoiner.create((SIRContainer) parentStream,
					SIRJoinType.NULL, 2);
		else if (joiner.getIdent().equals("WEIGHTED_ROUND_ROBIN"))
			joinType = SIRJoiner.createWeightedRR((SIRContainer) parentStream,
					joiner.getArgs());
		else
			at.dms.util.Utils.fail(printLine(type) + "Unsupported Join Type");
			...
			for (int i = 0; i < params.length; i++) {
				if (self.ident.equals(paramNames[i])) {
					return new JLocalVariableExpression(
							params[i].getTokenReference(), params[i]);
				}
			}
		if (supportedType(left.getType().getCClass().getIdent())) {
			return new JStringLiteral(self.getTokenReference(), left.getType()
					.getCClass().getIdent());
		}
		...
		if (left instanceof JFieldAccessExpression) {
			if (((JFieldAccessExpression) left).getIdent().equals("input")) {
				...
				if (v.elementAt(0) instanceof JClassExpression)
					filter.setInputType(((JClassExpression) v.elementAt(0))
							.getClassType());
				else if (v.elementAt(0) instanceof JStringLiteral)
					filter.setInputType(getType(((JStringLiteral) v
							.elementAt(0)).stringValue()));
				else if (v.elementAt(0) instanceof JMethodCallExpression) {
					// defining an array to be passed over the tape...
					JMethodCallExpression methCall = (JMethodCallExpression) v
							.elementAt(0);
					if (methCall.getIdent().equals("getClass")) {
						// array, record the dimension
						if (methCall.getPrefix().getType() instanceof CArrayType)
					...
					Utils.fail(printLine(self)
							+ "Malformed type on input declaration");
				...
				if (v.size() > 2) {
					...
					// Otherwise set the peeks to the number of pops
					filter.setPeek((JExpression) v.elementAt(1));
				return null;
			} else if (((JFieldAccessExpression) left).getIdent().equals(
					"output")) {
				if (!(parentStream instanceof SIRFilter))
					at.dms.util.Utils.fail(printLine(self)
				...
				if (v.elementAt(0) instanceof JClassExpression)
					filter.setOutputType(((JClassExpression) v.elementAt(0))
							.getClassType());
				else if (v.elementAt(0) instanceof JStringLiteral) {
					filter.setOutputType(getType(((JStringLiteral) v
							.elementAt(0)).stringValue()));
				} else if (v.elementAt(0) instanceof JMethodCallExpression) {
					// defining an array to be passed over the tape...
					JMethodCallExpression methCall = (JMethodCallExpression) v
					...
					Utils.fail(printLine(self)
							+ "Malformed type on output declaration");
				return null;
			}
	...
	public Object visitConstructorCall(JConstructorCall self,
		...
		for (int i = 0; i < params.length; i++)
			params[i] = (JExpression) params[i].accept(this);
		return new JConstructorCall(self.getTokenReference(), functorIsThis,
				params);
	}
	...
	/**
	 * Returns a vector of all the JInterfaceDeclarations for the toplevel
	 * stream
	 */
	public JInterfaceDeclaration[] getInterfaces() {
		JInterfaceDeclaration[] ret = interfaceList
				.toArray(new JInterfaceDeclaration[0]);
		return ret;
	}

	/**
	 * Returns a vector of all the SIRInterfaceTables that were constructed in
	 * traversing the toplevel stream
	 */
	public SIRInterfaceTable[] getInterfaceTables() {
		SIRInterfaceTable[] ret = interfaceTableList
				.toArray(new SIRInterfaceTable[0]);
		return ret;
	}
	...
	public Object visitTryCatchStatement(JTryCatchStatement self,
		...
		/*
		 * tryClause.accept(this); for (int i = 0; i < catchClauses.length; i++)
	...
	public Object visitTryFinallyStatement(JTryFinallyStatement self,
		...
		/*
		 * tryClause.accept(this); finallyClause.accept(this);
		 */
		return self;
	}
	...
	public Object visitSynchronizedStatement(JSynchronizedStatement self,
		...
		/*
		 * cond.accept(this); body.accept(this);
		 */
		return self;
	}
	...
	protected void deepCloneInto(at.dms.kjc.Kopi2SIR other) {
		...
		other.application = (at.dms.kjc.JCompilationUnit[]) at.dms.kjc.AutoCloner
				.cloneToplevel(this.application);
		other.parentStream = this.parentStream;
		other.topLevel = this.topLevel;
		other.trash = this.trash;
		other.num = this.num;
		other.lineNumber = this.lineNumber;
		other.currentMethod = (java.lang.String) at.dms.kjc.AutoCloner
				.cloneToplevel(this.currentMethod);
		...
		other.helperList = (java.util.Vector) at.dms.kjc.AutoCloner
				.cloneToplevel(this.helperList);
		other.global = (at.dms.kjc.sir.SIRGlobal) at.dms.kjc.AutoCloner
				.cloneToplevel(this.global);
		other.searchList = (java.util.LinkedList) at.dms.kjc.AutoCloner
				.cloneToplevel(this.searchList);
		other.params = (at.dms.kjc.JFormalParameter[]) at.dms.kjc.AutoCloner
				.cloneToplevel(this.params);
		other.paramNames = (java.lang.String[]) at.dms.kjc.AutoCloner
				.cloneToplevel(this.paramNames);
		other.finalVars = (java.util.LinkedList) at.dms.kjc.AutoCloner
				.cloneToplevel(this.finalVars);
		other.anonCreation = this.anonCreation;
		other.nextLatency = (at.dms.kjc.sir.SIRLatency) at.dms.kjc.AutoCloner
				.cloneToplevel(this.nextLatency);
