...
public class WriterOutputContent extends OutputContent implements DeepCloneable {
     ...
     * 
     * @param filter
     *            The SIRWriter used to contruct the FileOutputContent.
     */
    ...
    public JStatement closeFile() {
        JMethodCallExpression close = new JMethodCallExpression(null,
                new JThisExpression(null), closeMethod.getName(),
                JExpression.EMPTY);
    ...
    /**
     * Create kopi code that when translated to C will manipulate the file. The
     * C file will need to include <stdio.h>
     */
    @Override
    public void createContent() {
        ...
        JVariableDefinition fileDefn = new JVariableDefinition(null, 0,
                new CEmittedTextType("FILE *"), fileVar, null);
        // create fields
        JFieldDeclaration file = new JFieldDeclaration(null, fileDefn, null,
                null);
        this.addAField(file);
        ...
        JExpression[] params = { new JStringLiteral(null, getFileName()),
                new JStringLiteral(null, "w") };

        JMethodCallExpression fopen = new JMethodCallExpression(null,
                new JThisExpression(null), "fopen", params);
        // assign to the file handle
        JAssignmentExpression fass = new JAssignmentExpression(null,
                new JFieldAccessExpression(null, new JThisExpression(null),
                        fileDefn.getIdent()), fopen);

        initBlock.addStatement(new JExpressionStatement(null, fass, null));

        // do some standard C error checking here.
        // do we need to put this in a separate method to allow subclass to
        // override?
        ...
        JMethodDeclaration initMethod = new JMethodDeclaration(null,
                Constants.ACC_PUBLIC, CStdType.Void, "init_filewrite"
                        + my_unique_ID, JFormalParameter.EMPTY,
                CClassType.EMPTY, initBlock, null, null);
        ...
        workBlock.addStatement(new JExpressionStatement(
                new JAssignmentExpression(tmp.getRef(), pop)));
        ...
        if (KjcOptions.asciifileio) {
            ...
            fprintfParams[0] = new JFieldAccessExpression(null,
                    new JThisExpression(null), fileDefn.getIdent());
            ...
            // the first parameter: &(variable); treat the & operator as a
            // function call
            JExpression[] addressofParameters = new JExpression[1];

            addressofParameters[0] = tmp.getRef();

            JMethodCallExpression addressofCall = new JMethodCallExpression(
                    null, "&", addressofParameters);
            ...
            sizeofParameters[0] = new JLocalVariableExpression(
                    null,
                    new JVariableDefinition(
                            null,
                            0,
                            CStdType.Integer,
                            (getInputType().isFloatingPoint() ? "float" : "int"),
                            null));

            JMethodCallExpression sizeofCall = new JMethodCallExpression(null,
                    "sizeof", sizeofParameters);
            ...
            fwriteParams[3] = new JFieldAccessExpression(null,
                    new JThisExpression(null), fileDefn.getIdent());

            JMethodCallExpression fwrite = new JMethodCallExpression(null,
                    new JThisExpression(null), "fwrite", fwriteParams);
        ...
        JMethodDeclaration workMethod = new JMethodDeclaration(null,
                Constants.ACC_PUBLIC, CStdType.Void, "work_filewrite"
                        + my_unique_ID, JFormalParameter.EMPTY,
                CClassType.EMPTY, workBlock, null, null);
        ...
        JMethodCallExpression close = new JMethodCallExpression(null,
                new JThisExpression(null), "fclose",
                new JExpression[] { new JFieldAccessExpression(null,
                        new JThisExpression(null), fileDefn.getIdent()) });
        JBlock body = new JBlock();
        body.addStatement(new JExpressionStatement(close));

        closeMethod = new JMethodDeclaration(null, Constants.ACC_PUBLIC,
                CStdType.Void, "close_filewrite" + my_unique_ID,
                JFormalParameter.EMPTY, CClassType.EMPTY, body, null, null);
     ...
     * 
     * @return true if format is floating point
     */
    public boolean isFP() {
