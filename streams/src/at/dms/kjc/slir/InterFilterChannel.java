...
/**
 *  An InterFilterChannel represents an edge in the partitioned stream graph between slices.
 *  But it actually connects {@link OutputNode}s to {@link InputNodes}.
 * 
 * @author mgordon
 *
 */
public class InterFilterChannel extends Channel implements at.dms.kjc.DeepCloneable, Comparable<InterFilterChannel>{
    private static HashMap<ChannelDescriptor, InterFilterChannel> edges =
        new HashMap<ChannelDescriptor, InterFilterChannel>();
    
    /**
     * No argument constructor, FOR AUTOMATIC CLONING ONLY.
     */
    private InterFilterChannel() {
        super();
    }
    
    /**
     * Full constructor, (type will be inferred from src / dest).
     * @param src   Source of directed edge as OutputNode
     * @param dest  Destination of directed edga as InputNode
     */
    public InterFilterChannel(OutputNode src, InputNode dest) {
        super(src,dest);
        
        //make sure we did not create this edge before!
        ChannelDescriptor edgeDscr = new ChannelDescriptor(src, dest);      
     ...
     * @param src 
     */
    public InterFilterChannel(OutputNode src) {
        super();
        this.src = src;        
    }
     ...
     * @param dest
     */
    public InterFilterChannel(InputNode dest) {
        super();
        this.dest = dest;
    }

    public static InterFilterChannel getChannel(OutputNode src, InputNode dest) {
        ChannelDescriptor edgeDscr = new ChannelDescriptor(src, dest);      
    ...
    public void setSrc(InternalFilterNode src) {
        ...
        //make sure we did not create this edge before!
        ChannelDescriptor edgeDscr = new ChannelDescriptor((OutputNode)src, getDest());      
        InterFilterChannel edge = edges.get(edgeDscr);
        assert (edge == null) : "trying to create 2 identical edges";
        //remember this edge
        edges.put(edgeDscr, this);
    ...
    public void setDest(InternalFilterNode dest) {
        ...
        //make sure we did not create this edge before!
        ChannelDescriptor edgeDscr = new ChannelDescriptor(getSrc(), (InputNode)dest);      
        InterFilterChannel edge = edges.get(edgeDscr);
        assert (edge == null) : "trying to create 2 identical edges";
        //remember this edge
        edges.put(edgeDscr, this);
    ...
    public int initItems() {
        ...
        //System.out.println(next.initItemsReceived()  + " * " + ((InputNode)dest).ratio(this));
    ...
    public int steadyItems() {
        ...
        FilterInfo next = FilterInfo.getFilterInfo(((InputNode)dest).getWorkNode());
    ...
    */
    public int primePumpItems() {
        return (int) ((double) FilterInfo.getFilterInfo(((OutputNode)src).getWorkNode())
     ...
     * @return -1, 0, 1
     */
    public int compareTo(InterFilterChannel other) {
    ...
    private static class ChannelDescriptor {
        ...
        public ChannelDescriptor(OutputNode src, InputNode dest) {
            this.src = src;
            this.dest = dest;
        }
        
        public ChannelDescriptor(Filter src, Filter dest) {
            this(src.getOutputNode(), dest.getInputNode());
        }

        public ChannelDescriptor(InterFilterChannel edge) {
            this(edge.getSrc(), edge.getDest());
        }
        
        public boolean equals(Object obj) {
            if(obj instanceof ChannelDescriptor) {
                ChannelDescriptor edge = (ChannelDescriptor)obj;
