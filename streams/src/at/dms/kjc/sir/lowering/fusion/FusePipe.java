...
/**
 * This is the high-level fusion class for pipelines. It dispatches to another
 * class (e.g., FusePipeShift, FusePipeModulo) for actually performing the
 * fusion. This class deals with the interface, the legality of fusion, and
 * patching the fusion product into the parent stream graph.
 */
public class FusePipe {
	...
	/**
	 * Fuses all eligibles portions of <pipe>, returning the number of filters
	 * eliminated.
	 */
	...
	/**
	 * Fuses all candidate portions of <pipe>, given that the caller would
	 * prefer not to eliminate more than <targetElim> filters from <pipe>. The
	 * fusion does its best to respect this constraint (in a greedy way for
	 * now), but it will error on the side of eliminating MORE than <targetElim>
	 * instead of eliminating fewer.
	 */
	...
	/**
	 * Fuses sections of <pipe> according to <partitions>, which specifies the
	 * grouping sequence of who gets fused together.
	 */
	public static SIRStream fuse(SIRPipeline pipe, PartitionGroup partitions) {
		...
		SIRPipeline newPipe = RefactorPipeline.addHierarchicalChildren(pipe,
				partitions);
	...
	/**
	 * Fuses two filters starting from <start> in <pipe>, returning 1 if they
	 * were fused and 0 otherwise.
	 */
	public static int fuseTwo(SIRPipeline pipe, int start) {
		if (isFusable(pipe.get(start)) && isFusable(pipe.get(start + 1))) {
			fuse((SIRFilter) pipe.get(start), (SIRFilter) pipe.get(start + 1));
			return 1;
		} else {
			return 0;
		}
	 ...
	 * 
	 * 1. all of <first> ... <last> are consecutive filters in their parent,
	 * which must be an SIRPipeline
	 */
	public static void fuse(SIRFilter first, SIRFilter last) {
		...
		if (firstIndex == 0 && lastIndex == parent.size() - 1) {
			...
			SIRPipeline newPipe = new SIRPipeline(parent, parent.getIdent()
					+ "_Cont");
	...
	/**
	 * Fuses all candidate portions of <pipe>, but only fusing in segments of
	 * length <maxLength>, eliminating a maximum of <maxElim>. Candidates for
	 * fusion are sequences of filters that do not have special,
	 * compiler-defined work functions. Return how many filters were ELIMINATED
	 * from this pipeline.
	 */
	private static int internalFuse(SIRPipeline pipe, int maxLength, int maxElim) {
		...
		do {
			...
			if (end > start) {
				assert pipe.get(start).getParent() == pipe : "This stream ("
						+ pipe.get(start) + " thinks its parent is "
						+ pipe.get(start).getParent()
						+ " even though it should be " + pipe;
				assert pipe.get(start).getParent() == pipe;
				fuse((SIRFilter) pipe.get(start), (SIRFilter) pipe.get(end));
	 ...
	 * 
	 * Fuses ALL children of pipe, requiring that they are fusable filters.
	 * Leaves just a single filter <f>, which will be <pipe>'s only child
	 * following this call.
	 */
	private static void internalFuse(SIRPipeline pipe) {
		...
		for (int i = 0; i < pipe.size(); i++) {
			assert isFusable(pipe.get(i)) : "Trying to fuse a filter that is unfusable: "
	...
	/**
	 * Returns how many filters in this can be eliminated in a fusion operation.
	 */
	...
	/**
	 * Returns whether or note <b>str</b> is a candidate component for fusion.
	 * XXX: the current implementation only identifies fusable filters and
	 * rejects fusable containers. XXX: the calls from FuseSimpleSlit and
	...
	public static boolean isFusable(SIRStream str) {
		...
		if (filter.getWork() == null) {
			if (debugging) {
				System.err.println("filter " + filter.getName()
						+ " is not fusable because has no work fn");
			}
			return false;
		}
		...
		if (receivesFrom.length >= 1 || sendsTo.length >= 1) {
			if (debugging) {
				System.err.println("filter " + filter.getName()
						+ " is not fusable because has messages");
			}
			return false;
		}
	...
	/**
	 * In <parent>, replace all children with <fused>, and add arguments
	 * <initArgs> to call to <fused>'s init function.
	 */
	static void replace(SIRPipeline parent, SIRFilter fused, List initArgs) {
	 ...
	 * <filterInfo>. <filterInfo> must be a list of either SIRFilter's or
	 * FilterInfo's.
	 */
	public static String getFusedName(List<SIRStream> filterInfo) {
		StringBuffer name = new StringBuffer("Fused");
		for (ListIterator<SIRStream> it = filterInfo.listIterator(); it
				.hasNext();) {
			name.append("_");
			Object o = it.next();
