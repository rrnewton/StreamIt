...
public class CommonPasses {
    ...
    private SIRStream doStaticPassSIRStream(SIRStream str, int ssgNum) {
        ...
        if (KjcOptions.fusion || KjcOptions.dup >= 1 || KjcOptions.noswpipe) {
            ...
            StreamItDot.printGraph(
                    str,
                    "before-vectorize-ssg" + ssgNum + ".dot");
            SimplifyPopPeekPush.simplify(str);
            VectorizeEnable.vectorizeEnable(
                    str,
                    null);
            vectorizedEarly = true;
            StreamItDot.printGraph(
                    str,
                    "after-vectorize-ssg" + ssgNum + ".dot");
        }
        ...
        if (KjcOptions.tilera != -1) {
            ...
            if (!at.dms.kjc.tilera.TMD.allLevelsFit(
                    str,
            ...
            if (KjcOptions.optfile == null)
                str = at.dms.kjc.smp.SMPBackend.scheduler.SIRFusion(
                        str,
                        KjcOptions.smp);
        ...
        StreamItDot.printGraph(
                str,
                "canonical-graph-ssg" + ssgNum + ".dot");
        ...
        StreamItDot.printGraph(
                str,
                "before-partition-ssg" + ssgNum + ".dot");
        ...
        StreamItDot.printGraph(
                str,
                "after-partition-ssg" + ssgNum + ".dot");
        ...
        if (str instanceof SIRFilter) {
            System.out.println("CommonPasses.doStaticPassSIRStream after str="
                    + str.getName() + " isStateful="
                    + ((SIRFilter) str).isStateful());
        }
     ...
     * 
     * @param str
     *            The SIRStream from which to remove the filters.
     * @return The modified SIRStream
     */
    private SIRStream removeDummies(SIRStream str) {
        ...
        if (str instanceof SIRPipeline) {
            ...
            // Special case here. The pipeline should not consist of a single
            // node.
            // If it does, there will be problems in the fusion optimization
            // code.
    ...
    public StreamGraph run(SIRStream str, JInterfaceDeclaration[] interfaces,
        ...
        associatedGlobals = StaticsProp.propagate(
                str,
                theStatics);
        ConstantProp.propagateAndUnroll(str);
        ConstantProp.propagateAndUnroll(
                str,
                true);
        ...
        System.out.println("Creating segmented SIR graph...");
        SegmentedSIRGraph segmented = new SegmentedSIRGraph().init(
                str,
                result.isDynamic());
