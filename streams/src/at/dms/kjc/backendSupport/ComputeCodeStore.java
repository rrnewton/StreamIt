 ...
 * 
 * A Code Store implements a SIRCodeUnit so that you can add fields and methods
 * as needed. A ComputeCodeStore also has a main method (of type void->void).
 * The main method has three predefined parts:
 * {@link #addInitStatement(JStatement)} can be used to add a statement that is
 * performed before the steady state. This is normally called to add statments
 * calling (or inlining) a work method that needs to be run at initialization
 * time to pre-load data needed for peeks.
 * {@link #addSteadyLoopStatement(JStatement)} can be used to add a statement
 * that is performed in the steady state loop.
 * {@link #addCleanupStatement(JStatement)} can be used to add a statement that
 * is performed after the steady state loop. It may be useful to call
 * {@link #addInitFunctionCall(JMethodDeclaration)} to add a call to some method
 * to the beginning of the main method. As the name implies, this is usually
 * used to add calls to the init() functions in filters.
 * 
 * Use constructors without the iterationBound parameter to run the steady state
 * indefinitely (and thus make {@link #addCleanupStatement(JStatement)}
 * useless). Use constructors with the iterationBound parameter to run the
 * steady state for the number of times determined at run time by the value of
 * the iterationBound.
 * 
 * As lifted from MGordon's code there is a mutually-recursive type problem:
 * ComputeCodeStore and ComputeNode types are extended in parallel, and each
 * refers to the other. New constructors allow ComputeCodeStore to be
 * constructed without reference to a ComputeNode.
 * 
 * @author mgordon / dimock
 * 
 * @param <ComputeNodeType>
 *            nasty recursive type, historical relic. TODO: get rid of.
 */
public class ComputeCodeStore<ComputeNodeType extends ComputeNode<?>>
		implements SIRCodeUnit {

	/**
	 * The name of the main function for each tile. This static field is passed
	 * to {@link #setMyMainName(String)} which can use it or ignore it in
	 * setting up a local name for the main loop in each ComputeCodeStore.
	 * Assuming that there is only a single subclass of ComputeCodeStore in a
	 * given back end you could attempt to override this local field by:
	 * 
	 * <pre>
	 * static {
	 * 	if (&quot;&quot;.equals(mainName))
	 ...
	 * </pre>
	 * 
	 * But you would need to understand your class loader to know what the value
	 * would be if multiple subclasses of ComputeCodeStore are loaded in a back
	...
	/**
	 * set to false if you do not want to generate the work functions calls or
	 * inline them useful for debugging
	 */
	...
	/**
	 * Constructor: steady state loops indefinitely, code store has pointer back
	 * to a compute node.
	 * 
	 * @param parent
	 *            a ComputeNode.
	 ...
	 * 
	 * @param stmt
	 *            statement to add after any other statements in cleanup code.
	 */
	public void addCleanupStatement(JStatement stmt) {
		if (stmt != null)
			cleanupBlock.addStatement(stmt);
	}

	/**
	 * Adds field <b>field</b> to this, if <b>field</b> is not already
	 * registered as a field of this. (existing field is checked for with ==).
	 */
	@Override
	...
	/**
	 * Add the call the init function for a filter as the first statement in
	 * "main".
	 * 
	 * @param init
	...
	/**
	 * @param stmt
	 *            statement to add after any other statements in init code.
	 * 
	 */
	...
	/**
	 * Bill's code adds method <b>meth</b> to this, if <b>meth</b> is not
	 * already registered as a method of this. Requires that <b>method</b> is
	 * non-null.
	 */
	@Override
	...
	/**
	 * Add a way of iterating steadyLoop to the main method. This version takes
	 * a local variable (add declaration to mainMethod) that will hold the
	 * iteration count. Current implementation is a simple for loop counting up.
	 * XXX: The proper behavior is to loop forever if this variable is set to
	 * -1, but the current implementation will loop only INT_MAX times.
	 * 
	 * @param iterationBound
	...
	/**
	 * Return the main function of this store that will execute the init,
	 * primepump, and loop the steady-schedule.
	 * 
